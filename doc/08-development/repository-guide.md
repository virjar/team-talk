...
client/headless          JVM ImBot、常驻 Agent、CLI 与 MCP
...
client/desktop-bootstrap Desktop 原生安装壳中的 JVM 负载启动与切换
server/server            JVM 服务端
server/admin             React/Vite 管理后台（构建产物由 server 分发）
```


**包名约定**：自有 Kotlin 产品代码按职责使用 `com.virjar.tk.<module>`（如 `com.virjar.tk.protocol`、
...
顶层其余目录：`scripts/`（构建、CI、E2E 与静态资源辅助）、`doc/`（稳定产品和工程文档）、
`buildSrc/`（构建与部署插件）。
...
├── client/         ImClient、RpcClient、ClientSession、EventProcessor、LocalCache
...
ImBot 位于 `client/headless/.../bot/`，与 `agent/` 下的 Agent、CLI、MCP 共用 SDK。
无头安装与运行入口见[无头客户端](../05-clients/headless.md)。
...
├── env/                 data root
├── runtime/             Application 资源所有者、准入与关闭
└── di/                  Koin module
```
...
`application/PresenceCoordinator.kt` 负责连接状态与 presence 通知的协调。
设备登录信息的查询端口归 `domain/auth/DeviceRepository.kt`，与认证生命周期同址；
...
- `buildSrc/src/main/kotlin/release/`：统一密封清单、跨平台打包和发布通道。
...
| 客户端 SDK | `client/shared/src/commonTest` / `jvmTest` / `androidUnitTest` |
| 无头 Agent、CLI、MCP 和 ImBot | `client/headless/src/test` |
| E2E 夹具 | `client/shared-testkit/src/commonMain`（缓存、RPC 等替身，永不作为产品依赖） |
| 共享 UI | `client/app/src/commonTest` / `desktopTest`（状态、编辑与真实 SDK 组合回归） |
| 服务端集成/e2e | `server/server/src/test`（integration / e2e） |
| 真实部署验收 | `server/server/src/test/.../e2e/RemoteAcceptanceTest`，由独立 `acceptanceTest` 任务运行 |
| 远端容量基线 | `server/server/src/capacityTest`，按容量任务显式运行 |
| Desktop / Android 壳 | `client/desktop/src/desktopTest` / `client/android/src/test` |
| Desktop 负载启动与切换 | `client/desktop-bootstrap/src/test` |
