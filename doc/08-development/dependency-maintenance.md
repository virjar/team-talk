...
JavaCV 的 Java API、FFmpeg Java 绑定和各平台 native classifier 必须按对应 POM 一起核对。当前
JavaCV `1.5.12` 依赖 FFmpeg `7.1.1-1.5.12`；版本目录与各 classifier 使用同一版本。升级时用
`dependencyInsight` 同时核对声明与解析结果，避免传递依赖冲突将显式版本静默提升。
