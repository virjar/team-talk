...
| 本地缓存生命周期 | 部分 | 消息/媒体有界回收、损坏隔离、隔离期间附件源保留、只读 SQLite 诊断与 JVM 离线单库压缩已实现；Android 压缩、旧 namespace 处置及显式救援/放弃仍缺。细节见下方同名说明。 |
...
不足均拒绝。Android 仍只有只读诊断，旧 namespace 保留/回收、显式救援/放弃尚未完成。定向验证入口见
