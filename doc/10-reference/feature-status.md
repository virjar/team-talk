...
TeamTalk 当前发行版本为 0.0.3，处于开发者预览阶段，协议为 0.3。本文描述当前源码能力，并区分规划、实现与实际验证范围；发行变更与升级边界见[发行说明](../07-operations/releases/0.0.3.md)。
...
| 能力 | 状态 | 当前边界 |
|---|---|---|
| 数字协议版本与兼容窗口 | 可用 | 正式 0.0.3 冻结协议 0.3、最低支持 0.0；TCP 先协商再 AUTH，服务端最低版本可提高；旧客户端横幅提示或强制拒绝工作区。协议 0.1 新增携带账号范围的封禁帧，保留原认证响应布局 |
