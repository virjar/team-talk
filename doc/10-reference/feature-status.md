...
| 能力 | 状态 | 当前边界 |
|---|---|---|
| 数字协议版本与兼容窗口 | 可用 | 当前协议 0.2、最低支持 0.0，独立于统一展示版本 0.0.0；TCP 先协商再 AUTH，服务端最低版本可提高；旧客户端横幅提示或强制拒绝工作区。协议 0.2 新增携带账号范围的封禁帧，保留原认证响应布局 |
...
| 能力 | 状态 | 当前边界 |
| --- | --- | --- |
| Desktop 应用壳与三栏布局 | 可用 | 应用级标题栏、全局搜索、聊天主体和临时右侧检查器已分层；macOS 关闭隐藏后点击 Dock 可恢复并聚焦主窗口，与托盘“打开”共用恢复入口，支持取消最小化 |
