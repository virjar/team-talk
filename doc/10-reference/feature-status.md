...
| 私聊、群聊和群成员管理 | 可用 | 建群和邀请链接创建使用客户端稳定 operationId 与服务端持久收据覆盖丢响应重试；GUI 在 RPC 前按 deployment + uid 持久化冻结命令，可跨进程恢复；邀请回执 7 天内每创建者最多 256 条、不淘汰未过期身份且重放重新校验当前 admin，包含角色、禁言、转让群主；双端邀请管理提供完整链接复制，会话/通讯录“加入群”支持同部署链接及旧邀请码预览、确认加入和已加入时打开；浏览器落地页提供操作说明，尚不自动唤起客户端 |
