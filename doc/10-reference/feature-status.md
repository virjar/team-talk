...
| 本地缓存生命周期 | 基础可用 | 消息/媒体有界回收、可靠事实保留、损坏隔离、只读诊断、资料保全与显式放弃、Android 应用内整理及 JVM 离线压缩已实现，并提供明确范围的聊天/文档救援。高级救援扩展按实际需求另行立项，不作为基础能力的完成前置；交付故障和设备复验由 REL-05 承接。具体已支持与未支持范围见下方同名说明。 |
...
定向救援有各自的准入与重放边界，基础可用不代表任意损坏都能恢复，也不代表全平台交付验收完成。
...
保留，首次附件失效不保证成功；其余文档操作不在该入口范围内，见
[可靠文档创建救援](../03-architecture/client-and-sdk.md#可靠文档创建救援)。
空间创建救援恢复唯一冻结空间请求及同空间全部草稿与已准入文档命令，嵌套 creating 祖先须有原准入
命令，各路径独立合法且整个创建依赖图无环；不同时刻的旧路径原样保留，无需父子前缀一致。
原 ID、请求和后继修改保持，其他空间普通草稿留在来源。当前可见空间按 ID
收尾；祖先确认持久收尾并发布有效投影后，只衔接依赖该祖先且其余依赖已满足的已有命令，含隔着
已有中间父的后代，不重试无关失败兄弟。pending/未准入祖先阻止后代续发。
空投影当次不衔接，后续刷新或显式保存沿原命令接受服务端裁决；本地祖先不作授权依据。已改空间
名称不回滚，无命令叶草稿和后继修改不自动保存。其他结构操作与外部未提交创建依赖不在该入口范围内，单文档
入口的唯一标签/命令限制不变，见
[空间创建与文档树救援](../03-architecture/client-and-sdk.md#空间创建与文档树救援)。
已有空间创建组合通过独立入口按 `spaceId` 恢复全部未退役草稿与冻结命令，来源不能带任何待确认空间
创建/删除/归档，选中空间须有冻结命令。全局身份、草稿正文/资产和冻结内容先校验，选中链不得指向已知其他空间节点；
其他空间独立 creating 工作留源。目标整个文档 namespace 仍须为空，正常启动在当前可用空间沿原命令
及祖先依赖重放，后继修改和无命令草稿不自动保存，见
[已有空间文档树创建救援](../03-architecture/client-and-sdk.md#已有空间文档树创建救援)。

当前不支持多空间一次导入、非空目标合并、外部未提交创建依赖、其他业务命令/Bot 队列救援、文档
移动/改名/删除/归档操作救援、文档未完成本机上传，以及 Android 文档来源和 Android 原机导入。
这些属于实际需求触发后另行立项的高级救援候选，不作为基础生命周期的完成前置。已有归档、隔离与
未知资料继续按各自保留规则处理；不以零计数授权清理，不跨 dataset 自动重放。

交付制品的故障恢复、设备与平台复验统一见[REL-05](roadmap.md#rel-05--发布物晋级门禁)，按实际承诺
范围执行；既有定向测试不等于磁盘耗尽、断电和广泛真机组合均已验证。

定向验证入口见[已有空间文档树创建救援](../09-testing/local-tests.md#已有空间文档树创建救援)、
[空间创建与文档树救援](../09-testing/local-tests.md#空间创建与文档树救援)、
