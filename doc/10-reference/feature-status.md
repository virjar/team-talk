...
| 能力 | 状态 | 当前边界 |
| --- | --- | --- |
| Desktop 应用壳与三栏布局 | 可用 | 应用级标题栏、全局搜索、聊天主体和临时右侧检查器已分层；macOS 关闭隐藏后点击 Dock 可恢复并聚焦主窗口。Windows 已整合实机发现的用户身份/父目录 ACL、无 fileKey 文本读取与托盘中文菜单修复；新增 Windows CI 目录与打包检查，整合后的 Windows 发行产物仍需实机复验。启动失败支持复制诊断详情 |
