...
| 能力 | 状态 | 当前边界 |
|---|---|---|
| 数字协议版本与兼容窗口 | 可用 | 初始协议 0.0、最低支持 0.0，独立于统一展示版本 0.0.0；TCP 先协商再 AUTH，服务端最低版本可提高；旧客户端横幅提示或强制拒绝工作区 |
...
| 统一发行工具链 | 部分 | 根版本、人工说明和冻结协议快照进入 Gradle 校验；`release` 密封 Android、三平台 Desktop 站点与 Server ZIP，可向本地、SFTP 站点和 GitHub 交付；服务器仍人工部署。新的私有应用可通过 `private-first` 模式保持当前版本首次分发，独立冻结协议契约并保留站点不可覆盖检查。Windows 使用同一任务，完整跨平台安装与更新验收仍需按参与平台执行，见[发行流程](../07-operations/releasing.md)。 |
