...
| 本地缓存生命周期 | 部分 | 消息/媒体有界回收、损坏隔离、附件源保留、只读诊断、资料保全/校验、隔离与 namespace 显式放弃、Android 应用内整理及 JVM 离线压缩已实现；归档单聊天草稿或单条 outgoing 可救到同 owner JVM 库，Desktop 可离线恢复无待确认操作的独立文档单标签。Android 放弃仅支持导出副本，业务/Bot 队列、文档操作与 Android 原机导入等救援仍缺。细节见下方同名说明。 |
...
保存，显式保存继续受服务端 CAS、权限与附件可用性约束。Android 来源、未完成本机上传、文档操作与
Android 应用内导入不支持，见[独立文档单标签救援](../03-architecture/client-and-sdk.md#独立文档单标签救援)。
定向验证入口见[独立文档单标签救援](../09-testing/local-tests.md#独立文档单标签救援)、
