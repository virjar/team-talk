...
TeamTalk 当前发行版本为 0.0.4，处于开发者预览阶段，发行协议为 0.3，源码待发布协议为 0.4。本文描述当前源码能力，并区分规划、实现与实际验证范围；发行变更与升级边界见[发行说明](../07-operations/releases/0.0.4.md)。
...
- **基础可用**：明确的基础范围已形成闭环，高级扩展按实际需求另行立项，不表示所有扩展或平台验收完成；
...
| 能力 | 状态 | 当前边界 |
|---|---|---|
| 数字协议版本与兼容窗口 | 可用 | 正式 0.0.4 使用冻结协议 0.3，源码待发布协议 0.4，最低支持 0.0；TCP 先协商再 AUTH，服务端最低版本可提高；旧客户端横幅提示或强制拒绝工作区。协议 0.1 新增携带账号范围的封禁帧，保留原认证响应布局 |
...
以下历史“双端”验证指 Android/Desktop。iOS 已接入同一 SDK 与共享业务界面，
原生网络、SQLite、Keychain、媒体、权限与 APNs 实现见 [iOS](../05-clients/ios.md)；
Apple 设备的构建、交互及推送验收独立记录，不继承双端的既有验证结论。
...
| 私聊、群聊和群成员管理 | 可用 | 建群和邀请链接创建使用客户端稳定 operationId 与服务端持久收据覆盖丢响应重试；GUI 在 RPC 前按 deployment + uid 持久化冻结命令，可跨进程恢复；邀请回执 7 天内每创建者最多 256 条、不淘汰未过期身份且重放重新校验当前 admin，包含角色、禁言、转让群主；双端邀请管理提供完整链接复制，聊天链接点击与前台剪贴板识别进入预览，支持同部署链接及旧邀请码、确认加入和已加入时打开；浏览器落地页提供操作说明，尚不自动唤起客户端 |
| 固定系统账号与服务号 | 可用 | 文件传输助手/服务号账号幂等引导、系统私聊与帮助回复已有实现；固定系统账号已与人类会话投影及容量分离；指令回复与原消息同批持久化，失败或中断后由幂等重放与启动恢复按冻结身份补发（CODE-01 已闭合）；服务号官方触达已具备：新用户欢迎语按稳定身份自愈补发、管理台维护模板，管理员可随时全员广播（台账+幂等重推+启动续跑），消息走服务号会话与正常投递链 |
| 组织架构与成员归属 | 可用 | 单组织树、多部门归属与受管群已接通；目录按 revision 分页收敛，支持离线旧投影，权限仍由服务端裁决。细节见下方同名说明。 |
| 受管部门群 | 可用 | 节点子树是成员事实源；组织变更和启动恢复自动收敛，拒绝手工成员修改 |
| 会话列表、草稿、置顶、静音、已读 | 可用 | 会话设置多设备同步；草稿与已读支持本地 outbox 和恢复；READY 完整草稿使用独立版本契约同步，未上传源只在本机恢复。桌面与 Android 会话菜单支持本机「标为未读」（进会话或标记已读后清除），Desktop 聊天头部「···」打开会话设置窗口（发起群聊、本机清空聊天记录）。细节见下方同名说明。 |
...
Desktop 右键与 Android 长按会话菜单均可切换免打扰；菜单另提供本机「标为未读」——无未读时显示，
叠加为徽标 1，是纯本地显示事实（`conversation_local_flag` 表），不参与协议契约，任何本机读取推进
（进入会话、标记已读）自动清除。Desktop 聊天头部「···」打开独立会话设置窗口：发起群聊（私聊预选
当前好友）与本机清空聊天记录；清空删除本机消息、回应与已结算外发回执，并持久化清空水位
（`cleared_before_seq`）——历史拉取页与迟到的低水位事件不再落库，重启后仍生效；对端与其他设备
不受影响，进行中的发送与已读水位语义不变。完整聊天草稿在本机 SQLite 保存 Markdown + sidecar、
...
| 附件存储治理 | 基础可用 | FileStore 提供全局/上传者硬配额、未引用租约和安全 GC；按用户决策不再扩展账号/组织维度的配额与保留期治理（原 REL-06 已从路线图移除） |
...
| 文档 @ 提及 | 可用 | 文档正文与引用块输入 `@` 弹出好友候选补全（按名字/username/uid 过滤），选中以标准 `mention://` Markdown 链接写回并随保存与版本持久；编辑态与预览态点击 @ 均打开用户资料卡。本期无服务端提及通知；候选来自好友投影，无匹配时不弹层 |
| 明暗主题 | 可用 | 共享令牌，平台持久化主题选择 |
| 真实头像 | 可用 | 用户头像使用强类型 FileStore `Attachment`；双端系统 picker 自动居中裁成不超过 512×512 的方形 PNG，经认证媒体缓存展示，支持上传、替换、清除、进度和失败回落。群管理员可设置/清除群头像，当前引用纳入附件保留图，SDK 持久投影支持离线冷启动；无自定义头像时回退成员拼图（1 人全幅、2 人对分、3 人上 1 下 2、4+ 人 2×2）与群名占位。投影和生命周期见[客户端与 SDK](../03-architecture/client-and-sdk.md#64-用户联系人和群成员投影) |
...
| 本地缓存生命周期 | 基础可用 | 消息/媒体有界回收、可靠事实保留、损坏隔离、只读诊断、资料保全与显式放弃、Android 应用内整理及 JVM 离线压缩已实现；当前不支持从归档导入或重放业务。交付故障和设备复验由 REL-05 承接。具体已支持与未支持范围见下方同名说明。 |
| Android 专项体验 | 部分 | 核心业务及文档图片/文件的系统 picker、二进制粘贴和物理键盘粘贴可用；系统 picker/权限/返回路径、最低无障碍语义和发行制品真机矩阵仍须按目标设备覆盖 |
| Android 后台 Push 与系统通知 | 部分 | 小米、华为、荣耀、OPPO、vivo、魅族六家官方通知的部署 Profile、设备注册、持久待通知记录、用户授权与点击会话接入已实现；服务端按当前未读、静音及访问范围筛选，客户端避免 TCP 与厂商重复提醒。全部厂商的真实送达仍需已开通包名、审核通道/分类及目标真机验收（六家代码均已就绪，尚未有任何一家完成真机验收）。未配置时保留进程存活期间的标准通知，详见[Android 通知](../05-clients/android.md#消息通知的当前范围) |
...
基础能力覆盖可回拉缓存的有界回收、可靠本地事实保留、损坏保全、只读诊断及明确范围的维护处置。
当前不支持从归档导入或重放业务；基础可用不代表任意损坏都能恢复，也不代表全平台交付验收完成。
架构与操作边界见[缓存生命周期架构](../03-architecture/client-and-sdk.md#67-schema损坏隔离与回收)。
...
| 能力 | 状态 | 当前边界 |
| --- | --- | --- |
| 群共享文件空间 | 部分 | 目录、版本、权限、配额、六命令（含移动）outbox/receipt、持久变更投影和文件名搜索已落地；群聊非图片附件在消息提交时自动归档到“聊天文件”文件夹（与消息共享物理附件、引用独立计数，删除互不影响；图片不归档；失败不影响消息发送），双端群文件界面支持移动到任意目录；历史/收据治理未完成；离职盘点已接入管理台（按群只读聚合，交接动作待产品语义界定）。细节见下方同名说明。 |
| 企业文档 | 部分 | 多空间、权限、树、正文/修订、离线投影、可靠移动、持久变更事件、文档级评论、全文搜索与整空间 markdown/zip 导出（含资产与相对链接，空间责任人与超级管理员可导出，后台开关缺省关闭）已实现；管理台离职资产面板支持盘点、指纹确认与批量交接。Desktop 客户端与 Android 模拟器已验证文档创建、搜索和打开；细节见下方同名说明。 |
...
| 待办与任务 | 可用 | 群固定卡片、执行人横幅、日历重复、文档/文件材料、Markdown 与延期指标已实现并验证；Android 文件选择器返回的表单恢复已通过模拟后台进程回收实测。不含通用重复规则、子任务、看板或任务搜索。通知与恢复的验证范围见下方说明。 |
...
群固定卡片、执行人横幅、Markdown、文档与附件、有理由延期、完成统计和每 N 周/月重复可用。
业务规则集中在[待办任务](../02-product/tasks.md)，UI 入口见[任务操作与材料](../05-clients/README.md#任务操作与材料)。

真实 Desktop 与 Android 模拟器覆盖群摘要、成员访问、材料上传下载、延期历史与完成；
日历规则覆盖双周、月末不足日号与重开后的日期保持。Android 系统文件选择器期间的后台进程回收恢复
保留输入与返回附件，服务端核对创建不重复。表单未保存时不是持久草稿，不承诺任意重启或未完成上传续传。

通知验证仅覆盖有效连接、进程存活且权限允许的范围。Android 模拟器签名包在 `Unrestricted` 电池策略下
通过开始/截止通知和点击导航；默认 `Optimized` 策略可能阻止后台网络，物理真机与厂商唤醒尚未完整验证。
进程回收后的外部唤醒属于[Android 后台通知](roadmap.md#client-07--android-后台唤醒与系统通知)；
API 35 的一套 16 KB 模拟器出现过 ART 原生崩溃，持续观察通过不能代替根因闭环。

旧服务器只提供原单次任务能力；高级操作要求升级，不从旧列表页数伪造完整摘要。
保存后的操作共用持久命令队列，未知结果保留原身份，明确拒绝留给用户处理。
恢复边界见[任务投影](../03-architecture/client-and-sdk.md#68-任务投影可靠操作与提醒)，
验收入口见[任务协作](../09-testing/scenario-catalog.md#i-任务协作)。
