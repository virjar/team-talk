...
| 本地缓存生命周期 | 部分 | 消息/媒体有界回收、损坏隔离、隔离期间附件源保留、只读 SQLite 诊断、隔离资料保全归档/校验、显式放弃隔离副本、Android 应用内整理与 JVM 离线单库压缩已实现；Android 放弃仅支持导出副本，旧 namespace 处置及救援导入仍缺。细节见下方同名说明。 |
...
旧 namespace 保留/回收与救援导入尚未完成。定向验证入口见
