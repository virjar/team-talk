...
| 本地缓存生命周期 | 部分 | 消息/媒体有界回收、损坏隔离、附件源保留、只读诊断、资料保全/校验、隔离与 namespace 显式放弃、Android 应用内整理及 JVM 离线压缩已实现；归档聊天草稿、单条 outgoing、独立文档单标签、可靠文档创建或带唯一待确认空间请求的文档树可救到同 owner JVM 目标。Android 放弃仅支持导出副本，普通空间多文档创建组合、业务/Bot 队列、其余文档操作与 Android 原机导入等救援仍缺。细节见下方同名说明。 |
...
名称不回滚，无命令叶草稿和后继修改不自动保存。没有待确认空间请求的普通空间多文档创建组合、
其他结构操作与外部未提交创建依赖仍缺，单文档入口的唯一标签/命令限制不变，见
[空间创建与文档树救援](../03-architecture/client-and-sdk.md#空间创建与文档树救援)。
定向验证入口见[空间创建与文档树救援](../09-testing/local-tests.md#空间创建与文档树救援)、
