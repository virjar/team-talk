...
[TODO-01](roadmap.md#todo-01--待办任务) 核心实现 `6bf873ea` 的协议 215 项、SDK 61 项、App 17 项与
服务端 56 项定向回归已通过；最新日历扩展及相关修复通过协议 216 项、SDK 19 项、App 19 项、
Android 3 项与服务端 26 项。两组选择的测试范围不同，不累加为去重总数。

真实 Desktop / Android 已验证群固定卡片、执行人横幅、Markdown、两端上传与文档关联、跨成员下载、
两次有理由延期及次数/历史、完成与平均耗时、每周启停且保留当前期。
后续真实 Desktop 已验证月 31 日规则的本期 9 月 30 日、下期 10 月 31 日，重启后保持一致；Android
已验证双周本期 9 月 14 日、下期 9 月 28 日。Desktop 连续切换周/月并输入日期、间隔和时刻的三轮
回归通过；Android 同一存活进程在首轮通知点击回前台后，再次退后台仍收到第二轮开始和截止通知，
点击打开同一任务详情。签名 release 包在 API 35 模拟器上的复验需要将该应用的系统电池策略设为
`Unrestricted`；默认 `Optimized` 曾以 `APP_BACKGROUND` 阻断网络，虽服务端已持久记录提醒，客户端仍未即时收到。
这项验证不构成默认电池策略下的后台送达保证。

Android 模拟器在系统文件选择器前台时，以 `am kill` 回收后台应用并确认旧进程退出；返回后新进程
保留原标题、Markdown、文档引用与首个已完成附件，第二个选择的文件正常加入并保存成功；独立服务端
核对仅有一条任务、一条创建审计与一份命令收据，两个附件引用齐全。原始不完整
日期/周期输入的保留，以及已有 pending 或 ACK 时不重放新建表单，由 App 回归覆盖。这是模拟后台
进程回收实测。后续签名 release 包也通过系统实际 `LOW_MEMORY` 回收复验：选择器返回后无需编辑任何
字段，协议协商完成即恢复高级表单，Markdown、每 2 月的 31 日锚点、时刻和返回附件均保留；服务端
确认只创建一条任务、一条审计和一份命令收据。这些模拟器结果仍不代表物理真机已验证。

群固定卡片显示显式共享任务的完整未完成计数；个人横幅关注当前执行人已开始的未完成项，计数不取自已加载的一页。
协议 `shareToGroup` 缺省 false，旧任务的群关联本身不公开内容；新建表单选择群时默认勾选共享，可取消。
只有提交 `shareToGroup=true` 才增加当前群成员只读权，编辑旧任务不会自动改变原共享设置。

周期规则为每 N 周/月（1–12），以首次日期为稳定锚点，指定时区及同日开始/更晚截止。每月不足原日号
时落在月末，下月回原日号。创建时本期已开始且未截止则立即生成本期，否则选下一期；不补建已结束历史。
每期独立保存，后续停机只补最近漏期，旧期未完成可与新期并存。模板不提供
编辑，单期修改不改变未来，停用不取消已生成任务，重新启用沿原锚点从下个开始时点继续。
开始与截止各产生一次持久提醒；没有固定间隔重复催办，未完成状态由横幅持续呈现。

新任务支持 Markdown、文档引用和受管附件；旧任务按 TEXT 展示，原文不变。执行人延期须有原截止、新截止
和原因，原承诺、逐次变更、次数与耗时独立记录；旧历史未知时明确标注，不按当前修改时间反推。
完整产品边界见[领域模型](../02-product/domain-model.md#11-待办任务)，使用入口见[客户端说明](../05-clients/README.md#任务操作与材料)。

保存后的旧命令、扩展命令与系列启停共用 SDK 持久队列，未知结果保留原身份，明确拒绝留给用户处理。
未保存表单不作为持久草稿，未完成上传不承诺跨进程续传。Android 文件选择期间的表单与选择结果由
SavedState 恢复，不扩展为强行停止或任意重启恢复。连接旧服务器时保留原单次任务读写，高级查询与扩展操作要求升级，
不根据旧列表页数伪造摘要。恢复边界见[任务投影](../03-architecture/client-and-sdk.md#68-任务投影可靠操作与提醒)，
场景规范见[任务协作](../09-testing/scenario-catalog.md#i-任务协作)。

系统通知依赖应用进程存活、有效连接与系统权限；Android 进程回收后的外部唤醒仍归
[CLIENT-07](roadmap.md#client-07--android-后台唤醒与系统通知)。
本轮实现状态不代表物理真机后台、Desktop 托盘或正式安装包已经验收。
