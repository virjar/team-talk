...
| 本地缓存生命周期 | 部分 | 消息/媒体有界回收、损坏隔离、隔离期间附件源保留、只读 SQLite 诊断、隔离资料保全归档/校验与 JVM 离线单库压缩已实现；Android 压缩、旧 namespace 处置及救援导入/显式放弃仍缺。细节见下方同名说明。 |
...
Android 原地压缩、旧 namespace 保留/回收、救援导入与显式放弃尚未完成。定向验证入口见
