...
| Android 后台 Push 与系统通知 | 部分 | 小米官方通知的 Profile、设备注册、持久待通知记录、用户授权与点击会话接入已实现；服务端按当前未读、静音及访问范围筛选，客户端避免 TCP 与厂商重复提醒。真实厂商送达仍需已开通包名、审核通道/模板及目标真机验收；其他国产渠道尚未接入。未配置时保留进程存活期间的标准通知，详见[Android 通知](../05-clients/android.md#消息通知的当前范围) |
