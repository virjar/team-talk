...
TeamTalk 当前发行版本为 0.0.2，处于开发者预览阶段。本文描述当前源码能力，包含待发行 protocol 0.3 的群头像、系统会话与文档提及候选；已发布 0.0.2 的范围以[发行说明](../07-operations/releases/0.0.2.md)为准。本文区分规划、实现与实际验证范围。
...
| 能力 | 状态 | 当前边界 |
|---|---|---|
| 数字协议版本与兼容窗口 | 可用 | 源码待发行协议 0.3、最低支持 0.0，正式 0.0.2 冻结协议 0.2；TCP 先协商再 AUTH，服务端最低版本可提高；旧客户端横幅提示或强制拒绝工作区。协议 0.1 新增携带账号范围的封禁帧，保留原认证响应布局 |
...
| 固定系统账号与服务号 | 部分 | 文件传输助手/服务号账号幂等引导、系统私聊与帮助回复已有实现；服务号任务由运行时统一关闭，固定系统账号已与人类会话投影及容量分离；回复的持久恢复仍待闭合，见 [CODE-01](roadmap.md#code-01--代码结构与所有权收敛) |
...
| 企业文档 | 部分 | 多空间、权限、树、正文/修订、离线投影、可靠移动、持久变更事件、文档级评论、全文搜索与整空间 markdown/zip 导出（含资产与相对链接，空间责任人与超级管理员可导出，后台开关缺省关闭）已实现；图形化资产交接仍缺。Desktop 客户端与 Android 模拟器已验证文档创建、搜索和打开；细节见下方同名说明。 |
