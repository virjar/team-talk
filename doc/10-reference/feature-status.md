...
TeamTalk 当前发行版本为 0.0.5，处于开发者预览阶段，发行协议为 0.4，源码暂无待发布 minor。本文描述当前源码能力，并区分规划、实现与实际验证范围；发行变更与升级边界见[发行说明](../07-operations/releases/0.0.5.md)。
...
| 能力 | 状态 | 当前边界 |
|---|---|---|
| 数字协议版本与兼容窗口 | 可用 | 正式 0.0.5 使用冻结协议 0.4，最低支持 0.0，源码暂无待发布 minor；TCP 先协商再 AUTH，服务端最低版本可提高；旧客户端横幅提示或强制拒绝工作区。协议 0.1 新增携带账号范围的封禁帧，保留原认证响应布局 |
...
iOS 已通过 Apple Silicon 模拟器全链路验收（真实账号登录认证、事件同步、心跳保活与
主要页面浏览），列为开发者预览受支持平台；真机签名、APNs 推送与设备级验收尚未完成，
安装渠道与验收边界记录在 iOS 页，该页结论不继承双端验证，独立维护。
...
| 会话列表、草稿、置顶、静音、已读 | 可用 | 会话设置多设备同步；草稿与已读支持本地 outbox 和恢复；READY 完整草稿使用独立版本契约同步，未上传源只在本机恢复。桌面与 Android 会话菜单支持本机「标为未读」（进会话或标记已读后清除），Desktop 聊天头部「···」打开会话设置窗口：聊天记录搜索（关键词、聊天记录/图片及视频/文件/链接分类浏览，发送人与日历天粒度时间范围筛选，结果时间倒序可加载更多，点击跳转聊天定位；基于只读倒序历史分页在客户端过滤，服务端读取同样尊重本机清空水位）、发起群聊与本机清空聊天记录。细节见下方同名说明。 |
| 文本消息 | 可用 | 统一使用 `RICH_TEXT` / Markdown，只有一条 wire 分支；长正文折叠，桌面弹窗/移动端页面查看全文；远离最新的历史视口提供“回到最新”胶囊，阅读历史中出现新消息或有人 @ 我时提供“跳至未读”悬浮入口直达 |
...
| 图片、语音、视频与缩略图 | 部分 | 完整下载后在有界账号缓存播放，支持离线缓存命中；移动端附件面板多媒体入口收敛为「相册」（图/视频混选按类型分流）与「拍摄」（应用内相机：点按拍照、长按录像、60 秒上限、物理方向元数据）；画廊支持手动旋转与缩放。平台覆盖与系统信任签名、公证边界见下方说明 |
...
| 全局消息与用户搜索 | 可用 | 本地会话、联系人和远程消息/用户聚合；姓名搜索支持全拼与首字母拼音（服务端入库派生，@ 候选同源扩展，访客不暴露拼音边界） |
