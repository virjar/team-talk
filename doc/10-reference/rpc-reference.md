...
当前表描述源码契约：0.0.5 发行使用冻结的 protocol 0.4，源码暂无待发布 minor；正式事实以
...
设备制造商选择。空 `registrationId` 取消注册；服务端未配置该厂商或包名不匹配返回 `false`。
...
文档检索标题与当前正文，分词后的查询词按 AND 组合，可跨标题与正文共同满足；群文件和聊天附件
检索不区分大小写的字面文件名片段，文件类型依据当前
