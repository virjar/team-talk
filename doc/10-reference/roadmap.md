...
TeamTalk 当前发行是 [0.0.4 开发者预览](../07-operations/releases/0.0.4.md)，发行协议 0.3 已冻结，源码使用待发布协议 0.4。产品功能与支撑它的基础设施
...
后续需求按以下未完成工作和真实反馈选择；首次新增契约才开启下一 minor，同一批次共用它，
不按功能、提交或内测包继续递增。开发者预览发行不表示产品已经稳定或完成系统签名与公证。
...
| 已有用户流程可持续使用 | 修复聊天、群设置、组织可见性、附件导出、账号状态和平台生命周期的真实反馈；保留正常升级数据与已发行客户端兼容 | 按改动跑 Android / Desktop 短路径，涉及 iOS 的改动加跑 iOS 模拟器检查；不把只通过 SDK 测试写成 UI 已验收 |
...
### CLIENT-08 · iOS 平台验收与首发

Native SDK、共享 UI 的平台移植、Swift/Xcode 宿主和 APNs 已进入源码；
实现边界与可复现入口见 [iOS](../05-clients/ios.md)。保留现有发布 tag 的契约，
不为这次开发分支的中间状态增加迁移层，也不把 iOS 制品追加入已发布的 0.0.4。

剩余交付条件：在 Apple Silicon + 完整 Xcode 上运行 Native 测试、构建及运行 iPhone/iPad 模拟器，
再以签名真机完成登录、离线草稿/待发恢复、相册/文件/录音、后台切换和 APNs 点击场景。
实际分发使用选定组织的 Apple 签名与渠道，现有 Android/Desktop 下载注册中心不替代 iOS 分发。
...
固定系统账号已与人类 Conversation 投影分离：创建与消息恢复只维护人类会话，旧系统派生行由启动
迁移整理；归属与数据保留边界见[系统账号](../06-server/domain-services.md#14-系统账号)。
服务号回复的持久恢复已闭合（CODE-01）：回复与原消息同批持久化，幂等重放与启动恢复按冻结身份
补发至多一条，终态拒绝即结算；能力与边界移入[功能状态](feature-status.md)，不再挂账。
...
管理台“用户详情 → 离职资产”已落地：Document 盘点与批量交接的图形化流程（目标显式选择、
计划指纹确认、冲突重新盘点、回执展示），以及 GroupFile 的只读盘点（按群聚合该用户创建且仍活跃
的条目与版本字节；群文件属于群资产，不随文档交接转移）。资产由操作者明确交给指定人员或组织，
不推测 leader，也不因部门父链自动改变责任人。

剩余：GroupFile 是否需要交接动作（以及交给谁）取决于产品对“群内个人资产”的语义界定，
先保持只读盘点；其余业务域真实出现后再接入。
...
构建/服务端 JDK 21 和桌面壳内嵌 JBR 21 已落地，不再作为升级待办；Android 字节码兼容基线保持既定值。
Desktop 图标子集已进入实际桌面壳构建输入，不与未来整应用混淆、播放器升级混为一个任务。
常规维护按[依赖升级流程](../08-development/dependency-maintenance.md)执行，下列跨代工作单独评估，
不与产品功能绑定。候选版本在实施时核对稳定性与兼容矩阵，本页不推荐一个未经验证的“最新组合”。

| 工作包 | 剩余修改边界 | 完成条件 |
|---|---|---|
| `DEP-01` 构建与客户端工具链 | 评估下一代 Gradle/AGP/Kotlin/Compose/compileSdk 的兼容关系，核对 Android KMP 插件、DSL、KSP 和打包任务 | 服务端、SDK、Android/Desktop 可构建（启用 iOS 时含 iOS Native target）；协议生成一致；候选包启动及短路径通过；提高 compileSdk 不自动提高 targetSdk |
...
- RTC 通话、聊天文件夹、定时发送：先明确产品范围与使用优先级，不因其他软件已有而自动进入队列。
