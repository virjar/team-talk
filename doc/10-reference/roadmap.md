...
本地缓存生命周期的基础能力已具备，诊断、归档保全、显式放弃和压缩范围见[功能状态](feature-status.md#客户端体验)。
当前不支持从归档导入或重放业务；交付故障与设备复验由 REL-05 按承诺范围承接。
...
小米官方系统通知已具备客户端接入、部署 Profile、设备注册、服务端持久待通知记录及点击会话路径。
未配置厂商服务时保留进程存活期间的标准通知。小米不提供透传唤醒，消息内容在用户点击后沿现有登录与
权威同步获取，不新增后台登录或常驻服务。实现与配置见[Android 通知](../05-clients/android.md#消息通知的当前范围)。

剩余工作：使用实际包名开通的小米应用、官方大陆 SDK、私信通道和审核模板，完成真实厂商送达验收；
覆盖锁屏、系统回收、设备断网恢复、静音与已读、重复投递、通知点击、权限关闭和厂商不可用。
系统设置中的“强行停止”不作为必能唤醒的承诺。中国大陆其他手机厂商按目标用户设备接入官方渠道，
不接 Google/FCM，也不为尚无设备需求的渠道预造框架。未完成目标真机的厂商实际送达前，本项保持未完成。
