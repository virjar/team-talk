...
TeamTalk 当前发行是 [0.0.1 开发者预览](../07-operations/releases/0.0.1.md)。后续工作优先维护现有用户
流程、恢复能力和可读架构，再按真实使用需求选择新增产品能力。
...
Android 应用内可保留登录信息暂停会话，独立整理当前账号库后恢复，不丢弃草稿与待发；数据库族与逻辑库
各限 64 MiB，以限制原生临时库内存占用。进程持有维护状态，前后大小在设置中展示，边界见[当前会话数据库整理](../03-architecture/client-and-sdk.md#当前会话数据库整理)。

隔离资料已支持精确 owner 的原始保全导出与归档校验，连同替代库、附件源和独立文档资料保存；
校验不代表可恢复或可删除源，边界见[隔离资料保全归档](../03-architecture/client-and-sdk.md#隔离资料保全归档)。
独立的显式放弃入口要求完整归档与清单摘要确认，仅删除原字节匹配的隔离文件，保留替代库和共享资料，
支持同归档重试；JVM 操作现存安装，Android 只操作离线导出副本。后续源回收影响见
[显式放弃隔离副本](../03-architecture/client-and-sdk.md#显式放弃隔离副本)。

已知账号 namespace 默认保留，现可按部署指纹、datasetId 与 uid 显式保全并确认放弃，覆盖同 owner 全部
数据库代、隔离副本、独立文档资料与附件源；当前凭据引用或状态不明时拒绝。Android 只处理离线导出目录，
未知 legacy、共享 preferences、凭据与其他 owner 保留，不按年龄或零计数自动回收。完整边界见
[账号 namespace 保全与显式放弃](../03-architecture/client-and-sdk.md#账号-namespace-保全与显式放弃)。

单聊天草稿已可从经过校验的 JVM/Android 归档救到同部署、dataset 与 uid 的健康当前 JVM 库：
预览并确认目标时钟后导入完整 composer 及其本机附件源，先保留为待确认冲突稿，附件须手动重试，
不自动发送或同步。目标非空或存在可靠工作、源链有未确认提交依赖时拒绝，边界见
[单聊天草稿救援](../03-architecture/client-and-sdk.md#单聊天草稿救援)。

单条 outgoing 可通过独立预览与确认导入恢复原发送意图；活跃状态在客户端认证后续发原消息身份和
payload，终态失败保留，目标同身份或可靠工作冲突时拒绝。附件源与精确消费关联随选定消息核对，
不复制未知命令，见[单条 outgoing 救援](../03-architecture/client-and-sdk.md#单条-outgoing-救援)。

Desktop 独立文档单标签可从 format 1/2 JVM 归档离线预览并确认导入同 owner 的空文档 namespace。
源记录保持原身份、旧 saved/revision 基线和本机正文，打开不自动保存；所有归档数据库与目标库均须
排除待确认移动，源 manifest 也不得携带创建、删除或归档命令。显式保存继续由服务端 CAS 和当前资产
权限裁决，边界见[独立文档单标签救援](../03-architecture/client-and-sdk.md#独立文档单标签救援)。

可靠文档创建可从 JVM 归档中将唯一冻结请求与 creating 标签成对救到同 owner 空目标 namespace，
保留原 ID、原请求和后继草稿，拒绝其他结构操作依赖。确认导入后正常客户端在当前可见空间重放原请求，
后继修改不自动保存；已接受请求绑定原文档，首次创建仍受权限及资产可用性约束，边界见
[可靠文档创建救援](../03-architecture/client-and-sdk.md#可靠文档创建救援)。

单空间创建可与同空间全部草稿和已准入文档命令一起救援，包含原命令完整、各路径合法且创建依赖图
无环的嵌套 creating 文档树；不同时刻的原路径与后继修改保持，当前父链由服务端裁决。
正常启动按空间 ID 收尾或重放，祖先持久确认且发布有效投影后，只衔接依赖该祖先且其余依赖已满足
的已有命令，包括隔着已存在中间父节点的后代，不重试无关失败兄弟。无命令叶草稿不自动保存，空投影
当次不衔接，之后仍由服务端裁决。其他空间普通草稿
留在来源，边界见[空间创建与文档树救援](../03-architecture/client-and-sdk.md#空间创建与文档树救援)。

剩余为没有待确认空间请求的普通空间多文档创建组合、外部未提交创建依赖、其他业务命令、Bot 队列、
文档移动/改名/删除/归档等操作，以及 Android 文档来源和未完成本机附件上传等事实的救援；
当前入口不提供通用恢复或 Android 原机导入。后续救援须核对依赖与附件源所有权，不以替代库的零计数
推断旧资料可覆盖，不跨 dataset 自动重放；救援、显式放弃与 VACUUM 保持独立入口。
