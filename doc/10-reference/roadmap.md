...
单空间创建可与同空间草稿、直属 creating 标签和全部已准入文档命令一起救援，保留原请求与后继修改。
正常启动按当前空间 ID 收尾或重放，当前投影发布后衔接冻结子文档；无命令草稿不自动保存，空投影
不续发，其他空间普通草稿留在来源，边界见
[空间创建与直属文档救援](../03-architecture/client-and-sdk.md#空间创建与直属文档救援)。

剩余为其他业务命令、Bot 队列、嵌套 creating 文档依赖、文档移动/改名/删除/归档等操作，以及 Android 文档来源和未完成本机附件上传等事实的救援；
