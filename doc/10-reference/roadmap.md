...
| 剩余问题 | 修改边界 | 完成条件 |
|---|---|---|
| 服务号回复尚无持久恢复语义 | 原消息幂等命中直接返回，回复失败或进程死亡后不会随重试补齐。先明确回复与原消息 ACK 的关系，再选择最小恢复路径；已经成功的原消息不能因为发送者后续离群或禁用而变为失败，已有帮助消息也不能批量重发 | 同一命令只回复一次；原消息已提交、回复未完成时可按明确规则恢复；离群/禁用后的原消息幂等 ACK 维持原 seq，历史命令不会因升级重复回复 |

每项按独立变更验证真实 PostgreSQL/消息投影与失败恢复；涉及客户端可见结果时追加 Android/Desktop
短路径验收。恢复方案保持在服务号的明确边界内，不引入通用任务平台。
...
已确认的后续产品需求，排在当前重构之后推进；当前单次任务、手动消息卡片和到期提醒不等于下述能力
已经完成。产品范围与尚需定清的规则见[待办任务扩展需求](../02-product/domain-model.md#待办任务扩展需求待实现)。

| 交付切片 | 现有缺口与修改边界 | 验收结果 |
|---|---|---|
| 群聊固定卡片 | 群输入区域上方显示未完成待办摘要，增加服务端完整条件查询并确定群摘要可见范围；沿用群上下文，不以消息卡片替代 | 多任务聚合、完成/取消与离群后正确收敛，分页不导致漏计 |
| 执行人横幅 | 当前账号主要界面显示已开始的未完成待办，区分临期/逾期；复用事件与本地投影 | 改派、完成、取消、重新登录与通知已读后状态准确 |
| 周期待办 | 系列规则、时区、独立期次和开始/截止/催办计划；复用每期任务和服务端持久提醒 | 每周一按时提醒、当天下班截止；重启补偿且不重复，上期未完成与规则变更有明确结果 |
| 文档与文件附件 | 任务同时关联群上下文、文档引用和受管文件；保留各自资源权限和上传恢复 | 双端保存/重开可读，撤权、改派、上传失败和离线提交有明确反馈 |
| Markdown 描述 | 复用富文本渲染与编辑/预览，定义旧描述兼容 | 双端格式与附件展示一致，旧任务文字和保存重开不丢失 |
| 延期与统计 | 执行人填写延期原因，原承诺截止与逐次延期历史持久保存；明确处理耗时口径 | 多次延期/重试不重记，最终截止、最近延期时间、次数及平均处理耗时可追溯 |

实现时按实际依赖选择切片，并补齐协议、服务端、SDK 与双端闭环；群摘要、提醒与周期实例不能分别维护
互相矛盾的任务事实。普通升级保留既有任务与历史，未知的历史统计不伪造，不提前递增协议或正式版本。
