...
v0.0.3 已进入功能收口，范围见[发行准备稿](../07-operations/releases/0.0.3.md)。本批只处理影响既有功能的
缺陷、v0.0.2 升级兼容和制品验收；新增需求留待下一开发周期。整个批次统一使用 protocol minor 3，
不按功能、提交或内测包继续递增。v0.0.3 仍是开发者预览发行，不表示产品已经稳定或完成系统签名与公证。
...
**已完成。** 六项能力、双端主要流程及 Android 系统文件选择器返回的模拟后台进程回收恢复已验证。
产品规则与边界归入[待办任务扩展](../02-product/domain-model.md#待办任务扩展)，具体测试与平台范围见
[功能状态](feature-status.md#办公协作)，操作入口见[客户端说明](../05-clients/README.md#任务操作与材料)。
Android 进程回收后的外部通知唤醒继续由 [CLIENT-07](#client-07--android-后台唤醒与系统通知) 承接，
不将任意重启、未完成上传续传或物理真机后台覆盖纳入本项完成承诺。
