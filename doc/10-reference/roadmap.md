...
| `REL-03` 证书与 secret 轮换 | 长期私有节点需要在保留数据和客户端连接的前提下更换凭据 | 首次 HTTP + 自签 TCP 已实现；继续补轮换顺序、失败恢复与验收 |
...
首次 IP + HTTP + 自签 TLS/TCP 部署已经实现：Kotlin 配置注入公共证书，Gradle 生成或复用证书，
HTTP connector 与 TCP keystore 独立，客户端使用专用 TrustStore 并继续校验 SAN。构建配置与证书测试、
12 个真实 TLS 场景及双端编译通过；目标实例与正式交付包的结果仍按
[传输验收](../09-testing/deployment-acceptance.md#传输安全验收门槛)和 REL-05 记录。
普通升级保留原证书、私钥与数据，不自动重签。现有使用方式见
[传输配置边界](../07-operations/configuration.md#传输配置边界)，不再重复开发首次生成和固定信任流程。

**可交接下一步：单独完成长期凭据轮换与失败恢复。**

1. 定义服务器证书与客户端固定信任的更新顺序、兼容窗口和回退点，避免只替换服务器证书便使已安装
   客户端全部断连。沿 `DeploymentConfig`、`TlsDeploymentPreflight` 与 `ClientTransportTls` 维护一条
   可审阅链路；错误证书、过期和 SAN 失配仍须明确失败，不使用 trust-all 或关闭主机名检查。
2. 补齐数据库口令、管理凭据等非 TLS secret 的独立轮换，明确持久化、启用和失败恢复的先后关系；
   不通过清空数据库、重新生成全部 secret 或重新安装客户端完成轮换。
3. 用仍持有旧凭据的实例、轮换中的实例和更新后的客户端验证成功、失败及回退路径，保留账号、消息、
   草稿和发件箱，并核对健康与真实业务；结果写入验收报告，不把配置测试替代客户端升级验收。

首发不提前引入可信代理 CIDR、来源头或 PROXY protocol；代理可以 TCP 透传，应用不据来源头推断真实来源。
完成条件：证书、数据库口令与管理凭据可分别轮换，旧客户端过渡和失败恢复步骤明确，操作不丢数据，
健康与业务验收通过。
...
账号封禁清理还须使用同批发行制品验证在线封禁、旧凭据重连、密码错误不清理、中断后启动续清，
以及其他账号和系统导出文件保留。2026-09-07 双端开发构建已完成关键流程与 pending 恢复验收，
具体范围见[功能状态](feature-status.md)；共享层与服务端测试不能代替 Android 真机的生命周期验收。
...
客户端已实现权威封禁后的精确本地资料清理，见 CORE-06；它不删除服务器文件，也不替代这里的
资产交接、组织保留期和审计规则。
...
账号封禁已有独立的显式删除边界：只有权威封禁及可证明的 deployment + dataset + uid 才能清理，
HTTP 401 先 refresh 重验。持久 marker 先行，关闭账号资源后删除数据库族、隔离副本、草稿、媒体与
账号诊断与匹配凭据；启动在打开账号资源前续清未完成任务。安装配置、其他账号或数据集与
系统导出副本保留。这项策略不扩大普通凭据失效或容量 LRU 的删除范围，剩余失败交互和发行制品验证归
REL-05；实现图与源码入口见[账号封禁与本地资料清理](../03-architecture/client-and-sdk.md#211-账号封禁与本地资料清理)。
