...
聊天的本机跨进程草稿、私有有界源文件 spool、上传 outbox 和消息入队时原子消费草稿已有实现，
见[附件与富内容](feature-status.md#附件与富内容)。恢复只继续上传，消息仍由用户主动发送；Android
导入期间 Activity 重建的设备覆盖集中到 REL-05。

剩余是已 READY 服务器资产的跨设备草稿同步：为 Markdown + canonical sidecar 定义明确的草稿契约、
版本冲突与清空语义，并让另一设备恢复模式、回复目标与已就绪资产。不能把内部 URI 裸写入旧字符串
草稿协议，也不复制或同步本机尚未上传的源文件。

完成条件：同账号两台设备恢复 READY 富资产草稿；并发修改、清空和发送消费不会让迟到草稿复活，
接收设备提交前重新核验资产可用性。断网未上传的本地草稿继续由各安装独立持有。
