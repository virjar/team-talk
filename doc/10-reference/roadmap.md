...
### CLIENT-06 · 平台资源发布与首帧（已闭合）

媒体与草稿候选的 IO 构造、Main 组装、失败释放及退役责任交接已完成，Android 完整候选发布与
同 owner 草稿续接也已收敛。定向测试、双端编译及本地 Desktop 草稿重开/注销验证通过，
详见[客户端资源换代](feature-status.md#客户端体验)与[Desktop 架构图](../05-clients/desktop.md)。
后续交付复验集中到 REL-05；其他入口在发现具体阻塞或泄漏时单独处理。
