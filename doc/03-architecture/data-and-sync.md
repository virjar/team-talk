...
这一规则针对已验证的合法阅读意图。SDK 写入前核对同一会话的消息依据；旧客户端留下的越界本地
水位通过当前会话历史核实后校正，避免持续掩盖未读。持久依据与在途更新的处理见
[本地可靠事实](client-and-sdk.md#62-本地可靠事实与容量)，不能只按可能落后的 PostgreSQL 会话页截断水位。
...
服务端为六类 mutation（含移动）都保存 actor、kind 和规范 payload 指纹的不可变收据。创建/追加版本的精确重放会核对
条目或版本事实；重命名/移动/删除的 RPC 只返回 `Unit`，只有精确收据重放才作为无载荷结果的窄例外。
收据 ACK 只证明原命令已提交，不代表条目当前仍存在或可见。协议为重命名和删除补齐
`commandId`；移动（minor 4）沿用同一可靠命令契约，形成当前六类 mutation 的完整可靠命令集。
