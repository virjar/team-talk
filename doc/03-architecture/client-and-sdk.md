...
正常工作区重启在当前可见空间中继续原 ID、原冻结请求的创建待办，后继草稿和未提交标签不自动保存。
创建命令等待全部本地 creating 祖先；祖先持久收尾并发布有效投影后，只衔接依赖该祖先且其余依赖已满足
的既有命令，包括隔着已存在中间父节点的后代，不重试无关失败兄弟。空投影当次不衔接，后续刷新或显式
保存仍由服务端当前父链与权限裁决。已接受空间按可见 ID 收尾，不回滚其当前名称或改变用户导航。
...
没有可回收空闲页时不保证明显缩小。该操作不回收旧 namespace、不修复隔离库，也不执行资料放弃。CLI 用法见
