...
`ContentSearchRepository` 每次查询文档、群文件或聊天附件中的一个领域，摘要只驻留在当前搜索页，
不写入 SQLite、正文缓存或聊天历史。`ContentSearchFeature` 分别拥有三类分页窗口，每类最多 200 条；
关键词、来源、类型或范围变化会退役旧请求。网络错误保留明确的失败状态，不表示“没有结果”。

`EventProcessor` 为三类搜索分别维护固定大小的累计 generation。文档、群文件、消息与成员变化、组织
变化、重连和 reset 使相关查询失效，StateFlow 合并不会抹掉失效水位。第一页在飞行期间失效时最多重读
一次；续页不能接着旧游标拼接，当前页面从首页重新查询。搜索不预取全空间正文或文件。

打开前仍执行当前领域 RPC：文档通过 `getDocument`，群文件通过 `getEntry`；聊天附件用独立的
`MessageRpc.getHistory(chatId, serverSeq, 1)` 回读精确消息，核对同一 `scopeId + serverSeq` 下主附件
canonical path 的 SHA-256。这个读取不占用聊天仓储的更旧历史页链，也不覆盖驻留历史窗口。
真实对象解析成功后才交给平台导航或已有认证下载入口；撤权、删除、撤回或附件移除后的旧摘要不能
继续打开。搜索摘要不含 FileStore path，协议枚举主附件的规则由服务端与 SDK 共用。
...
账号 namespace 与隔离副本的显式放弃、聊天/独立文档救援和数据库整理使用下方各自独立入口；
高级救援的未支持范围见[功能状态](../10-reference/feature-status.md#客户端体验)，按实际需求另行立项。
...
本入口不恢复文档操作；单条可靠创建使用下方独立入口。Android 来源、未完成的本机附件上传和
Android 应用内导入仍不支持。命令见
[Desktop 独立文档草稿救援](../05-clients/desktop.md#独立文档草稿救援)，定向入口见
[独立文档单标签救援](../09-testing/local-tests.md#独立文档单标签救援)。

#### 可靠文档创建救援

Desktop 的文档创建救援从 format 1/2 JVM 归档中选择一个 creating 标签及其精确配对的冻结创建命令。
它复用文档 manifest、独立记录、墓碑、编解码与规范化规则；来源须使用当前文档 schema 11，归档内
同 owner 全部账号数据库须为 SQLite schema 6、dataset 一致且无待确认移动/改名。未退役记录中必须
恰有一条待确认文档创建和一个 creating 标签，不得带空间创建、删除或归档意图。其他普通标签也须
结构可读，以排除被损坏记录隐藏的创建依赖；选中标签与命令的身份和创建位置必须匹配，正文与资产
描述符须完整有效。不能丢掉未知依赖后只恢复其中一个记录。

目标仍为精确同部署、dataset 与 uid 的健康当前 JVM 库，客户端须退出，并取得现存安装锁与维护事务。
目标文档 namespace 只允许不存在、空目录或规范 DELETED 状态，当前库无待确认移动；不合并现有
标签或命令。预览输出不含正文的身份/范围摘要、归档清单摘要和目标状态摘要，确认导入时重新核对。
标签与冻结命令先写为完整记录，再由同一 manifest 原子发布；原归档、隔离资料与其他 owner 保持不变。

导入保留原文档 ID、标签恢复身份、冻结创建请求及用户随后编辑的草稿。确认该入口表示恢复原先已提交
的创建意图；正常启动后，在当前可见空间内由既有可靠创建流程重放原请求，不把后继草稿替换进在途命令。
若服务端已经接受该请求，无正文的幂等 ACK 用于绑定原文档身份，客户端不借此覆盖服务器当前正文。
后继草稿继续留在本机，须用户另行保存，并接受当前权限、附件可用性与 revision 检查。

空间撤权、归档或暂不可见时，创建待办可能继续保留；导入不保证立即提交或成功完成。首次创建仍须
通过服务端当前资产校验，归档里的附件已失效时可能失败，不自动上传替代附件或重造文档身份。
未确认结果始终保留原请求；普通无命令草稿继续使用单标签救援，不能借两个入口相互绕过依赖检查。

此单文档入口不处理空间创建及成组的 creating 依赖，空间与文档树使用下方独立入口。文档移动/改名、删除/归档等其他
命令、未完成本机上传及 Android 文档来源与原机导入仍不支持。命令见
[Desktop 可靠文档创建救援](../05-clients/desktop.md#可靠文档创建救援)，
验收入口见[可靠文档创建救援](../09-testing/local-tests.md#可靠文档创建救援)。

#### 空间创建与文档树救援

Desktop 的空间创建救援按 `space-command-<UUID>` 选择唯一未退役的冻结空间创建请求，一起恢复该
空间的全部草稿、creating 标签及其全部已准入文档创建命令，支持依赖完整的嵌套文档树。来源沿用 format 1/2 JVM 归档、
当前文档 schema 11 与同 owner 全部 SQLite schema 6 数据库的校验；所有源库与当前目标库均不得有
待确认移动/改名。不能只挑一份空替代库来忽略隔离资料中的依赖。

来源不得有其他待确认空间创建、其他空间的 creating 标签或文档创建命令、删除/归档意图。每个
creating 标签的路径单独校验：ID 为规范 UUID，祖先链最多 128 项，不含重复 ID 或自身，parent 为
祖先链末项；空间根文档的 parent 与祖先链均为空。某个祖先若也是本次恢复的 creating 标签，必须
已有原始准入命令；按全部本地 creating 祖先建立的创建依赖图必须无环，与运行时等待范围一致。
未提交父/祖先、非法路径或循环依赖均拒绝。

父子标签可能记录于不同时间，不要求其旧路径前缀一致，也不在导入时重写路径。已存在远端的父节点
保留原引用，当前父链与权限仍由服务端裁决，本地祖先信息不作为授权事实。

每条已准入命令须与标签精确匹配，冻结请求和后继草稿的正文、资产及身份均须完整有效。其他空间
的普通草稿留在来源，不随所选空间导入；不能修改清单或丢掉命令来绕过拒绝。

目标为精确同 deployment、dataset 与 uid 的健康当前 JVM 库，客户端已退出，文档 namespace
不存在、为空或只有规范 DELETED 清单。预览和导入复用现存安装锁、维护事务与两项摘要确认，完整
记录先写入，再原子发布 manifest；不覆盖目标已有标签或操作，不修改原归档、隔离资料或其他 owner。
导入保留空间及文档原 ID、原冻结请求与后继修改，不生成替代身份。

正常启动先按当前可见空间 ID 核对：空间已可见时收尾原空间创建待办；需要重放时，成功发布当前空间
投影后衔接已满足依赖的冻结文档创建。仍有 pending 创建命令或尚无命令的 creating 祖先时，子文档
保持等待。祖先文档确认已持久收尾且有效投影发布后，只唤醒依赖该刚确认祖先、且其余依赖均已满足的
既有命令；既包括直接子文档，也包括隔着已存在中间父节点的后代，不顺带重试无关的失败兄弟。
该衔接不改变用户后来选择的导航。已接受后又改名的空间保持当前名称，不用冻结旧名称回滚。

空间或父文档因归档、失权等只返回空投影时，当次不衔接子文档。后续刷新或显式保存仍使用原命令，
由服务端当前父链、权限和附件状态裁决，不能凭本地已收尾就认定子文档必然可创建。

无已准入命令的草稿和已准入请求之后的修改都不自动保存。失败或暂不可见时保留相关待办与本稿，
首次子文档创建仍可能因当前权限或附件失效而失败；导入完成不代表空间及文档已在服务端完成。
此入口不恢复未准入的父/祖先依赖、所选空间之外的创建依赖、其他结构操作、未完成本机附件上传或
Android 文档来源/原机导入。该入口以唯一待确认空间创建请求为前提；没有该请求的既有空间使用下方
独立文档树入口，单文档入口的唯一标签/命令限制保持不变。命令见
[Desktop 空间创建与文档树救援](../05-clients/desktop.md#空间创建与文档树救援)，
验收规范见[空间创建与文档树救援](../09-testing/local-tests.md#空间创建与文档树救援)。

#### 已有空间文档树创建救援

Desktop 的 `document-tree-create-rescue` 命令按 `spaceId` 选择已有空间，恢复该空间全部未退役草稿
及原冻结文档创建命令。来源不得包含任何待确认空间创建或删除/归档意图，选中空间至少有一条未退役
冻结创建命令。它接受 format 1/2 JVM 归档、当前文档 schema 11 与同 owner 全部 SQLite schema 6
数据库；所有源库和目标当前库仍须无待确认移动/改名。

选择之前严格校验所提供归档内全部未退役记录的身份、草稿正文/资产和冻结命令内容，空间筛选不会
跳过其中损坏的邻居记录或命令。
选中空间的草稿、命令与依赖完整保留；其他空间的独立 creating 工作和普通草稿留在来源，不一并导入。
若选中创建链指向归档中已知属于其他空间的节点则拒绝；未在归档中出现的远端父节点保留原引用，
当前父链和权限由服务端裁决。各路径合法性、已准入祖先要求与无环创建依赖图规则沿用上方空间救援，
不重写不同时刻记录的路径，也不把本地祖先当作权限事实。

目标须为精确同 deployment、dataset、uid 的健康当前 JVM 库，客户端退出且整个文档 namespace
为空或规范 DELETED 状态。现存安装锁、SQLite 排他维护与两项摘要确认保持不变；确认时重新核对
目标状态，完整记录先写入、manifest 最后原子发布。不合并已有目标工作区，也不只检查目标某个空间
是否为空；原归档、来源账号库及其他 owner 不变。

确认导入后正常启动，在选中空间当前可用时重放原文档创建请求，无需创建空间请求。运行时按既有
祖先依赖规则推进，确认持久收尾且有效投影发布后，只唤醒依赖已满足的相关后代；后继修改和无命令
草稿仍不自动保存。
导入成功不保证业务成功，服务端当前权限、父链和附件仍可拒绝首次创建，相关待办及本稿继续保留。
此入口不支持一次导入多个空间、非空目标合并、空间创建、移动/改名、删除/归档、未完成附件上传或
Android 文档来源/原机导入。旧单文档和待创建空间入口的范围不变。
命令见[Desktop 已有空间文档树创建救援](../05-clients/desktop.md#已有空间文档树创建救援)，
验收规范见[已有空间文档树创建救援](../09-testing/local-tests.md#已有空间文档树创建救援)。
