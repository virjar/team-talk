...
此单文档入口不处理空间创建，空间及直属文档使用下方独立入口。文档移动/改名、删除/归档等其他
...
空间的普通草稿、直属 creating 标签及其全部已准入文档创建命令。来源沿用 format 1/2 JVM 归档、
当前文档 schema 11 与同 owner 全部 SQLite schema 6 数据库的校验；所有源库与当前目标库均不得有
待确认移动/改名。不能只挑一份空替代库来忽略隔离资料中的依赖。

来源不得有其他待确认空间创建、其他空间的 creating 标签或文档创建命令、删除/归档意图，以及
creating 文档父节点或祖先依赖。选中空间内的 creating 标签限空间根直属文档；每条已准入命令须与
标签精确匹配，冻结请求和后继草稿的正文、资产及身份均须完整有效。其他空间的普通草稿留在来源，
不随所选空间导入；不能修改清单或丢掉命令来绕过拒绝。
...
投影后才衔接该空间的冻结子文档创建。该衔接不改变用户后来选择的导航。已接受后又改名的空间保持
当前名称，不用冻结旧名称回滚；归档或失权可能只取得空投影，此时不续发子文档创建。

无已准入命令的草稿和已准入请求之后的修改都不自动保存。失败或暂不可见时保留相关待办与本稿，
首次子文档创建仍可能因当前权限或附件失效而失败；导入完成不代表空间及文档已在服务端完成。
此入口不恢复嵌套 creating 文档依赖、其他结构操作、未完成本机附件上传或 Android 文档来源/原机导入。
命令见[Desktop 空间创建救援](../05-clients/desktop.md#空间创建救援)，
验收规范见[空间创建与直属文档救援](../09-testing/local-tests.md#空间创建与直属文档救援)。
