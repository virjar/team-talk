...
- “协作文档”从群设置进入，但编辑需要宽画布，因此切换到右栏主工作区，不塞进 400dp 检查器。
...
群设置中的“协作文档”打开宽屏工作区：一级导航保留，会话中栏暂时收起；工作区左侧 320dp 文档摘要
列表，右侧为标题、格式工具、正文编辑/预览与版本操作。返回聊天后恢复原会话中栏。列表响应不携带
所有正文，选中后再按 documentId 读取完整快照。

编辑器沿用聊天输入的 Markdown 富文本能力，但文档使用更大的连续画布。切换文档、新建、关闭编辑器
或退出工作区前，如果正文或标题未保存，必须显示放弃确认。保存按钮展示 revision 已保存/未保存状态；
冲突时保留本地编辑。历史窗口按需读取指定快照，“恢复”产生新 revision，不重写旧历史。

当前页面打开、保存或用户点击刷新时拉取数据；没有实时光标、后台推送或离线编辑。入口和文案必须
如实表达这个边界。
