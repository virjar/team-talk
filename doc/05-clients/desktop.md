...
- 左栏只负责一级栏目和设置入口。导航项为横向"图标+文字"行（钉钉/飞书桌面范式），
  使用一级导航专属矢量图标（TkNavIcons：气泡/人形/折角页/对勾板），保证剪影辨识度；
  选中态为 secondaryContainer 圆角底 + 主色实面图标 + 加粗标签。
- 中栏展示会话或联系人。
- 右栏保持当前工作上下文。

聊天面板的 Compose 状态与副作用绑定当前 `chatId` 和 `ChatViewModel`。切换会话时重新建立观察，
不能沿用上一会话的消息列表或已读目标；只有当前可见且窗口获得焦点的聊天才能自动推进已读水位。
会话列表的未读数字始终来自该会话自己的本地投影，切到别的聊天或窗口失焦都不能把新消息自动清零。
...
| 连续输入或选择任务 | 独立任务窗口 | 建群、转发、会话设置（聊天头部「···」进入，含聊天记录搜索、发起群聊与清空聊天记录） | 系统关闭、ESC |
...
各图形端好友资料提供“设置好友备注”，可以保存或留空清除，仅本人可见。联系人、资料标题、私聊会话与
...
- 输入器至少三行，Enter 发送，`Cmd/Ctrl+Enter` 换行（Markdown 源码模式相反：Enter 换行、`Cmd/Ctrl+Enter` 发送）。
...
| Cmd/Ctrl+K | 全局搜索 |
| Enter | 发送消息 |
| Cmd/Ctrl+Enter | 换行（Markdown 源码模式中为发送） |
