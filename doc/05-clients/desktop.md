...
归档中有冻结空间创建请求时，可将它与同空间全部草稿、creating 标签及已准入文档创建命令
一起导入，包括依赖完整的嵌套文档树。该入口沿用上方 JVM 来源、同 owner 健康目标库、客户端退出和空文档 namespace 要求；
...
来源须只有一个未退役空间创建请求，拒绝其他空间的创建待办或 creating 标签、删除/归档及移动/改名。
creating 祖先必须已有原准入命令，各标签路径独立合法且整体创建依赖无环；未提交父节点或非法路径
不导入。不同时刻记录的父子旧路径原样保留，不要求前缀一致；远端已有父节点保留原引用，最终由
服务端检查当前父链和权限。其他空间普通草稿留在原归档，所选
空间的原请求、原 ID、文档冻结请求和后继草稿分别保留，不把后继修改替换进已提交请求。

确认导入后，正常启动可重放空间请求；当前可见的同 ID 空间直接收尾，不回滚已改过的名称。空间
当前投影成功发布后才衔接已满足依赖的冻结文档；创建祖先仍未收尾时，子文档等待。祖先确认已
持久收尾且有效投影发布后，只续发依赖该祖先且其余依赖均已满足的既有命令，包括隔着已存在中间父
