...
> 管理客户端在本机用 256 位安全随机数生成一次性 `ttb_...` token；服务端只生成机器人身份和
> 入站通知 URL，并且永远不回传或保存可恢复的明文 token。
...
删除本地恢复副本。即使恢复时用户正在查看另一个群，机器人页也会展示这份凭据并明确标出原目标群，
不能形成“操作被锁住但没有处理入口”的隐藏状态。

`404`（目标已物理不存在）和 `410`（该精确凭据已被后续轮换、停用或达到永久账本边界）证明本次候选
token 不可能继续有效，客户端可以结束恢复。`409` 只表示 operationId 与 payload/token 冲突；原请求的
token 仍可能有效，因此与网络、超时、`403`、`429`、`5xx` 和本地响应解析失败一样，必须保留恢复副本。
页面允许用户经过二次风险确认后显式放弃；如果服务端其实已执行，放弃会永久丢失本机唯一副本，只能
在恢复管理权限后轮换或移除机器人。客户端按状态码做上述判断，不解析可变的错误文案。
...
每个机器人终身最多保留 256 条凭据命令收据（包含创建，因此最多轮换 255 次）。达到边界后应移除并
重新创建机器人，而不是删除旧收据后继续轮换；旧收据必须继续拒绝任意延迟重放。
