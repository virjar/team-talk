...
不再提供隔离/账号归档与显式放弃命令：缓存损坏由客户端自动移出旧库、重建干净库并在验证健康后
删除旧库族（见[损坏隔离与回收](../03-architecture/client-and-sdk.md#67-schema损坏隔离与回收)），
doctor 报告中出现的 `.corrupt-*` 目录属于恢复中途崩溃的残留，下一次恢复会自动清扫，无需处置。
