...
Markdown，因此不能为了“按钮多”伪造会在保存后消失的格式。文档遇到代码围栏、引用、表格、任务
清单、图片、原始 HTML、引用式链接等当前编辑器不能无损往返的结构时，自动使用 Markdown 源码模式；
标题修改和保存必须保留原始正文；预览器尚不能完整呈现时也显示带说明的完整只读源码，不能静默省略
段落。等这些结构进入富文本领域模型并具备往返测试后，才能开放对应按钮。
...
- 段落、标题、列表、引用和代码块；有序/无序列表、原始序号和嵌套层级不能在投影时丢失。
- 粗体、斜体、删除线、行内代码与链接。
