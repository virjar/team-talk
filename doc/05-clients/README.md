...
任务表单未保存的输入只驻留当前工作台会话内存，离开编辑器或重启不承诺恢复。点击保存后，完整
创建、编辑或状态命令先进入 SDK 持久队列，再由后台发送；入队不表示服务端已提交，未确认创建也不
立即读取一个尚不存在的远端对象。网络结果未知时保留原身份补发；403、409、410 等明确拒绝保留原
意图，用户可查看和复制“原内容”，再显式重试或放弃。放弃只移除本机命令，不删除服务器任务。
远端新修订不会覆盖正在编辑的输入，保存仍基于原修订检查冲突。

任务只有创建人与当前执行人可读；创建人管理信息、分配和取消，双方可在待处理、进行中、已完成
之间切换，取消后只有创建人可重新打开。群聊或部门仅是背景关联，离群或离开部门不撤销参与者身份。
截止时间使用本地日期和时间输入并转成 UTC，编辑其他字段保留原截止精度；到期事实由服务端产生，
应用内已读和系统通知已展示分别持久记录。权限与提醒规则见[领域模型](../02-product/domain-model.md)。
