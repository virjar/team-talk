...
`maxUses=0` 明确显示“不限次数”。接收方点击聊天中的邀请链接，或复制邀请链接/旧邀请码后返回已登录
客户端，即进入群预览；会话和通讯录标题不常驻低频的加入入口。剪贴板仅在应用获得前台焦点时读取文本，
未变化的邀请不会重复打开，普通文字与含多个不同邀请的内容不触发导航。查看群名与成员数后确认加入；
已加入成员显示“打开群聊”。预览不消耗次数，确认时由服务端重新判断
