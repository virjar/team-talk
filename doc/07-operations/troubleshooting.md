...
冻结空间请求与文档树需要一起恢复时，使用 [Desktop 空间创建与文档树救援](../05-clients/desktop.md#空间创建与文档树救援)，
以 `space-command-<UUID>` 列举、预览并确认两项摘要。唯一空间创建与完整同空间依赖一起导入，
其他空间普通草稿不带入；嵌套 creating 祖先须有原准入命令，各路径独立合法且创建依赖图无环。
不同时刻记录的父子旧路径无需前缀一致，不要手工统一旧路径。未提交父依赖、多空间创建或其他结构
操作拒绝，不拆掉依赖强行导入。当前空间可用后先重放依赖已满足的文档，祖先文档持久收尾并发布
有效投影后，只续发依赖该祖先且其余依赖已满足的既有命令，含隔着已存在中间父节点的后代；不重试
无关失败兄弟。祖先仍 pending 或未准入时，后代等待。
归档/失权返回空投影时，当次不衔接后代；后续刷新或显式保存使用原命令，由服务端当前父链与权限
决定结果，不把本地祖先当作授权。已接受空间当前名称不回滚，未提交草稿和后继修改不自动保存，
不把导入成功当作所有创建完成。

空间本身没有待确认创建请求，但存在多个冻结文档创建时，使用
[Desktop 已有空间文档树创建救援](../05-clients/desktop.md#已有空间文档树创建救援)。先列举候选空间，
再用 `--space-id` 预览并确认两项摘要；源空间创建和删除/归档意图必须全空，所有未退役记录的身份、
正文/资产及冻结内容仍须校验。其他空间独立创建可留源，选中链若指向已知其他空间节点则拒绝。目标要求整个文档
namespace 为空，不能因目标选中空间为空就合并已有工作区。启动后重放原命令，后继修改不自动保存；
不要伪造空间创建请求或拆分冻结命令来迁就旧入口。
