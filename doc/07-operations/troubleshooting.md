...
缓存损坏确认后，客户端把损坏库族原子移出原路径，在原路径重建干净库，服务端可回拉的投影由快照、
事件与各领域 RPC 重新收敛；替换库验证健康后损坏族随即删除，不再保留待处置副本。服务器是唯一可靠
信息源，损坏瞬间的未发送本地事实（草稿输入、未确认命令）不承诺恢复。诊断入口只读，不修改资料。

不再提供隔离/账号归档、校验与显式放弃命令。恢复中途崩溃残留的 `.corrupt-*` 目录属于弃用数据，
下一次恢复会尽力清扫；`doctor` 报告中看到它们时无需处置。正常账号库中的草稿、outbox 与上传任务
按各自流程恢复。
...
该入口原地 `VACUUM`，保留可靠事实；锁占用、非当前 schema/epoch、安装 major 不兼容和空间
不足会拒绝执行，不应删除锁或资料来绕过。
...
提示处理存储空间、数据库占用等原因后重试。Android 数据库族或逻辑库超过 64 MiB 时会在
