...
1. 调用 `deploymentConfiguration(rootDir)` 并由构造器校验最终字段；deployPath 必须是规范化的非根 POSIX 绝对路径，拒绝 `.`、`..`、
   重复分隔符和尾随 `/`。
...
4. 生成或加载本地 deployment.secrets；首次 HTTPS 或 HTTP + TLS/TCP 安装提供证书与私钥，在本地转换、
   读取 PKCS12 成功后才继续。HTTP 模式也接受成对证书参数，只启用 TCP TLS。
5. 创建 data、conf、ssl、static 等远端目录，上传分发文件、docker-compose、env.sh 与已准备的 TLS keystore。
...
确认值不是通用口令，必须逐字符等于当前部署配置函数返回对象中的
