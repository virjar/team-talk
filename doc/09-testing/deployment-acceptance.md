...
2. 使用真实界面注册、登录并发送中文消息，留下未发送草稿；服务端发布仅改动少量 jar 的新 build。
3. 客户端「设置 → 通用 → 检查更新」：显示目标版本；确认下载 → 实际传输进度 →
   「重启后生效」；重启后 `~/.teamtalk-client/<appId>/versions/current.properties`
   指向新 build，`设置` 内构建身份一致，自动登录、历史消息和未发送草稿保留。
4. 仅对测试客户端使用的 TeamTalk 更新端点限速；首个文件尚未完成时应已显示增量总量及持续变化的已下载字节。
   关闭下载对话框后客户端回 Idle，旧指针不变，无本次残留 `.staging-*`，立即重试不被取消的下载持锁阻挡；
   不关闭宿主机网络、全局代理、DNS 或 Agent 控制链路。
5. 截断更新文件的 HTTP 正文：明确失败、旧指针不变、暂存清理；恢复端点后通过界面重试成功。
6. 用户看到目标发布后、点击确认前切换或停用通道：提示重新检查，不能静默安装另一发布。
7. 通道指向保留的旧发布，经用户界面确认回退并重启后复查登录、历史和草稿。此项只验证所选客户端
   制品间的实际兼容，不代表服务端数据库可以随旧二进制直接降级。
8. dev/裸 JVM 运行（`./gradlew :client:desktop:run`）：设置面板不显示检查更新入口。
...
未发出的源文件和上传命令另见[聊天草稿与附件强杀恢复门禁](#聊天草稿与附件强杀恢复门禁)。
...
- `tt-agent` / `tt`：产品化无头客户端，适合长时间保持在线、外部自动化和跨客户端联动。

两者都必须通过公共 SDK 与真实服务交互，不允许绕过服务直接写数据库制造“成功”状态。需要验证服务端拒绝时，应从客户端 API 观察明确错误，而不是只检查日志。

## 无头分发与授权验收

使用隔离服务及专用验收账号，按[无头手册](../05-clients/headless.md)从实际解包的分发目录执行，
不依赖源码路径、Gradle 或 IDE classpath。账号 dataDir 和导出 token 必须位于安装 prefix 外，
使用本任务创建的私有目录；安装器测试可使用临时目录，实际 agent dataDir 仍须符合其专用目录策略。

| 场景 | 操作与预期 |
|---|---|
| 分发身份与移动 | 将分发移动到含空格的新路径，从另一个工作目录启动；核对 manifest、SHA256SUMS、版本/源码/协议和实际 launcher。三个 `--version` 不要求 Java 或网络，业务运行使用外部 Java 21；Windows 只验证 CLI，agent/MCP 应明确报告不支持 |
| 私有配置与重启 | `configure` 保存 TCP/HTTP 与可选公共证书，首次认证后只用 dataDir 重启；身份及已保存端点恢复，离线 doctor 不连接服务、不输出秘密。传入另一实例端点应拒绝并保留原凭据；对不存在目录运行 doctor 不创建它 |
| CLI 与 MCP 分离 | `export-cli-token` 保存私有管理员文件；创建具名 grant，显式指定 tools/chat。MCP 不接受管理员 token，也不回退读取 CLI 配置；scoped token 不能管理 grant，`tools/list` 只显示授权工具 |
| 会话和工具授权 | 允许会话历史可读，其他会话与未授权工具被拒绝；直接 REST 调用同样受限。限定范围的接收必须带允许的 chatId，会话列表不出现其他会话；需要全会话或账号级工具时显式授权 |
| 审计、撤销与重试 | 核对授权/工具审计仅含身份与状态，不含 token、正文或本地文件路径；撤销后旧 token 与已等待的接收结果均被拒绝，重启 agent 后撤销仍有效。grant HTTP 结果未知时复用同 ID/token/范围，发送结果未知时复用 clientMsgId 并查询回执，不能把撤销当作取消已入队消息 |
| 升级和卸载 | 使用安装所有者执行升级；原进程保持旧负载，新进程使用原子切换后的版本，原 dataDir/配置/可靠收发继续可用。活跃 agent/CLI/MCP、改写文件或未知内容使卸载拒绝；退出本安装所有进程后从外部解包目录卸载，仅受管安装消失，外部数据保留 |

审核输出和持久发送回执时区分 `queued/sending/failed/sent`；消息交付还须由另一真实 TeamTalk 客户端
确认正文和稳定 ID。只检查 MCP JSON-RPC 成功或 HTTP 200，不能证明消息已获得服务器 ACK。
Linux systemd 另检查 unit 使用预期非 root 身份、重启后读取已保存端点及正确版本，不用放宽数据目录权限
解决启动问题。完成后退出本任务启动的 agent、MCP 与验收对端，保留必要的脱敏结果，不留下隐形后台进程。
...
旁路读取，业务绑定后由当前引用授权：消息检查活动会话成员，文档检查当前空间 READ；完整聊天草稿
只授予该草稿 uid，且仍检查聊天访问权限。丢失全部有效业务引用权限的旧上传者同样得到 403。
匿名请求和无关用户必须分别得到 401 与 403。远端验收必须保留这些身份断言，防止随机路径或历史 uploader
身份重新退化为授权凭据。
...
本机聊天草稿与上传的跨进程恢复、READY 富资产草稿的同账号跨设备同步，按下面的专门门禁验收；
文档和消息编辑的临时上传不由该门禁覆盖。Android 文档拖放仍不属于已实现能力。

### 聊天草稿与附件强杀恢复门禁

只隔离专用 TeamTalk 服务端点，不关闭宿主机 Wi-Fi、网卡或修改全局网络配置。Android 使用明确的测试
设备序列号；Desktop 固定私有数据目录和实例令牌，强杀前核对该实例 PID。

1. 双端分别在已认证聊天中输入正文，经真实 picker/剪贴板导入图片和文件；停止专用服务形成离线窗口。
   确认本地草稿、资产 identity 和私有源已持久化，界面保留待处理项，未 READY 不能发送。
2. 强杀并重新打开相同安装和账号。检查正文、资产引用、模式、光标与回复目标恢复；仍离线时可以继续
   编辑和移除，恢复草稿本身不能产生已发送消息。
3. 恢复服务，检查未完成上传继续处理，重试使用同一固定字节源与上传 identity。修改或移动原始外部
   文件后，已保存源的上传内容不变；用户原文件不被清理。
4. 全部 READY 后显式发送。另一账号收到正文、图片及文件，历史中只有一个 `clientMsgId`，接收字节与
   已保存源一致；发送后重启原客户端，草稿为空且没有重复待发项。
5. 对另一个待处理资产，在离线时删除引用后强杀重启，再恢复服务。引用和待处理项不复活；持久源在
   最后引用被移除后释放。覆盖失败重试、取消及晚到完成的 attempt CAS，不能只看输入框视觉消失。

SQLite 集成用例另固定“入队事务前失败保留草稿、事务提交后同时存在消息且消费对应 revision、发送
期间的新草稿不被旧回执清空”。源文件用例覆盖容量拒绝、部分写清理、重启校验及活动读租约释放；
这些确定性窗口与真实双端短路径互补，不能用单一层的结果代替另一层。

### READY 聊天草稿跨设备门禁

同一实例、数据集与账号分别登录 Android 和 Desktop，固定测试设备与独立 Desktop 数据目录。
已有安装覆盖升级后先核对原账号、消息和本机草稿保留，不能清库后冒充升级结果。

1. 两端打开同一聊天，来源端通过真实输入及 picker/剪贴板建立含文字、图片、文件和回复目标的草稿。
   全部 READY 后，接收端无需切页即可恢复相同 Markdown、canonical sidecar、模式与回复身份；
   接收端没有原始源文件或上传 job。切换模式、继续编辑和清空均自动收敛，清空后 revision 稳定。
2. 只隔离专用服务端点，让一端持久化离线修改后退出；恢复服务并在另一端保存新稿，再重启旧端。
   旧端保留本机稿并显示冲突；“保留本机”与“使用其他设备”只在明确选择后推进或替换，不自动覆盖新稿。
3. 来源端退出后，接收端仍可预览 READY 资产并主动发送；提交前重新核验远端资产和草稿版本。
   消息正文、附件字节与回复目标正确，服务端历史只出现一个消息身份；重开来源端后草稿保持空。
4. SQLite/RPC 集成用例固定未知提交重试、并发清空、发送确认与下一份本机草稿的时序。
   条件清空只消费其对应消息的原草稿，失败、迟到 ACK 与另一端的新 revision 均不能删掉后继稿。
   尚未 READY 的本地源不跨设备发送，资产不可用或冲突应保留输入并显示可操作反馈。
