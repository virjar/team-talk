...
文档验收额外证明：列表不需要读取全部正文；群成员可从另一会话修订；旧 expectedRevision 必须失败；
修订 1 的完整 Markdown 在修订 2 后仍可读取；成员被移出群后不能继续读取；当前 revision 删除后文档
从普通列表消失。UI 的双栏/全屏布局、富文本输入、预览和未保存确认属于真实客户端验收，不由协议测试
替代。
