...
7 天未引用租约；消息、群文件、文档或完整聊天草稿引用必须在租约到期前权威提交，零引用过期对象会被服务端回收。
这条对象租约与上面的 identity/收据重试窗口是两个生命周期，客户端不能把一次上传成功理解为永久网盘
保留承诺。聊天富资产导入由账号 SQLite 和私有 spool 持有固定源、稳定 identity 与上传命令，重启和
认证恢复后继续上传；已过期 identity 的显式重试使用同一私有源创建新上传意图。上传 READY 不自动
发布消息，发送仍须通过正文/sidecar 的提交屏障。文档及已发送消息编辑的导入只在当前进程内重试。
附件 wire 保持原义。已 READY 资产的跨设备草稿使用独立 `ChatDraftRpc`，服务端按 uid 私有快照保存完整
sidecar 并持有文件引用，不传递设备上的待上传源；具体所有权见[富文本与媒体](../05-clients/rich-content.md)。
...
每次网络下载都必须使用 `Authorization: Bearer <accessToken>`。未被业务对象认领的暂存附件只允许上传者
读取；认领后按消息、群文件、文档、草稿或任务的实时权限裁决。当前个人头像对已认证用户可见，群头像
只对当前成员可见；知道随机 path 本身不构成权限。客户端缓存可以改变何时访问网络，但不
...
Range 是服务端下载能力，不改变客户端完整下载后再播放的约束。

撤回附件消息会移除其活动反向引用；删除群文件条目会移除该条目全部版本的活动引用。同一路径仍被
其他消息、群文件、文档、任务、草稿或当前头像使用时继续保留；周期模板的附件也计入引用。
全部业务引用消失后才具备物理回收资格，具体回收与暂存窗口见文件存储章节。
