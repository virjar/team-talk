...
| 身份 | 当前值与来源 | 用途 | 递增时机 |
|---|---|---|---|
| 统一展示版本 | `teamtalk.releaseVersion=0.0.0` | Server、SDK、Android、Desktop、MCP 使用同一字符串，配合 commit 排查构建范围 | 用户明确确认本次发行后推进；不从它推导协议能力 |
...
| 平台安装序号 | `teamtalk.releaseBuildNumber=0` | 安装器识别新包；Android `versionCode` 与 Conveyor `app.revision` 为 `buildNumber+1`，零号为 `1`；macOS jpackage 的系统包版本从 `1.0.0` 映射 | 用户确认分发新的安装包后独立递增；不参与协议判断 |
...
展示版本和安装序号的推进、人工发行说明定稿、`prepareProtocolRelease` 冻结、tag 与发布，必须等用户
明确确认本次发行后进行。开发构建、开发服务器部署和 Agent 本机验收不自动构成发版；工具提示版本或
快照不匹配也不是自行升版的依据。确认范围与执行流程见[发行决定](../07-operations/releasing.md#谁决定发版)。

用户确认后，一次发行由根版本配置命名，人工说明写入 `doc/07-operations/releases/<releaseVersion>.md` 并随配置
一起提交。公开仓库的 `v<releaseVersion>` tag 标记这次源码；私有客户没有 GitHub，也遵守同样的本地记录。
提交摘要只能帮助核对遗漏，不能覆盖人工撰写的变更、升级步骤和已知限制。

| 文件 | 表达的事实 | 可以怎样修改 |
|---|---|---|
| `gradle.properties` | 下一次构建使用的展示版本、安装序号与协议窗口 | 开发协议 minor 按实现演进；展示版本与安装序号仅在用户确认本次发行后推进 |
...
| `protocol/protocol/contracts/<major>.<minor>/` | 与展示版本无关的已分发 wire 契约 | 用户确认首次私有安装包分发后显式准备；同样不可覆盖、删除或回收编号 |

初始发行快照记录 `0.0.0 / buildNumber=0 / protocol=0.0` 的完整契约，源码身份由对应 tag 和密封清单固定。
发行前试验快照不作为本版兼容来源；初始基线确认后，不能再以“没有历史 tag”为理由回收已分发契约。
**向内测用户、SDK 集成方或任一私有客户分发都算发行**，须先获得用户对本次发行的确认并冻结契约，
不能以“没有公开下载”作为回收编号的依据。
...
flowchart LR
    ...
    Review --> Approval["用户明确确认本次发行"]
    Approval --> Consolidate["统一收敛为下一发行协议 0.4"]
...
不强迫每次调试都登记客户端发行快照。客户端统一 `release` 同时执行这项门禁与发行元数据检查，首次私有分发模式使用独立协议契约检查；真正向
用户分发时仍须先取得用户对本次发行的确认并冻结，不能以服务端调试路径绕过发行纪律。
