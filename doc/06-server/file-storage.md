...
消息、群文件、文档、用户头像和群头像都不信任客户端 sidecar。引用一个附件前共同执行：
...
完整聊天草稿保存同 uid 的私有 READY 引用。新增 path 必须来自本人 staging 上传，已有 path 只能从
同一草稿的当前清单复用，描述符仍需与真实元数据一致。该引用参与文件保留，但不向聊天其他成员授予
下载权；通过该引用读取时，同账号设备也必须仍能访问该聊天。清空或删除会话移除草稿引用后，文件按既有未引用 TTL
回收；发送消息仍须经过消息自身的权限校验和引用绑定。

任务的新材料也必须是操作者本人的 staging 上传，已有材料只按同一任务的完整描述符复用。
任务当前材料和固定周期模板分别持有保留引用；模板附件只负责支撑未来期次，不单独授权下载。
每次下载按实际任务的当前读权裁决，群共享不扩大被引用文档的空间权限；规则见[待办任务](../02-product/tasks.md)。
...
- path 属于本人完整聊天草稿的当前资产清单，且本人仍可访问对应聊天。
- path 属于当前可读任务的材料；读权来自创建人、当前执行人，或显式共享群的当前成员。

附件的反向引用来自当前 User/GroupChat 头像列、消息索引、群文件版本表、文档修订资产表、私有聊天草稿和任务资产表。当前群头像只允许该群现有成员读取；其引用同时进入总附件保留图，避免下载可见却被 GC 当作孤儿回收。消息引用随消息原子写入；群文件引用随文件版本、
...
上传响应成功不等于形成业务引用。未进入用户/群头像、消息、活动群文件、活动文档修订、聊天草稿或任务/周期模板引用的对象从 `uploadedAt` 起持有默认 168 小时
租约，配置 `TEAMTALK_UNREFERENCED_ATTACHMENT_TTL_HOURS` 可在 1–8760 小时内调整。小时级维护每次只
扫描固定页数、每页固定条数的 metadata keyspace，并批量查询 MessageStore 与 PostgreSQL 的头像、群文件、文档修订、草稿和任务/周期模板引用；有任一权威引用就
保留，零引用才进入 FileStore 已有的 `PENDING_DELETE`/原子 RocksDB 删除协议。消息撤回会原子移除
附件反向索引，群文件条目删除会让其全部历史版本退出活动引用；共享 path 只有最后一个引用消失后才
能被物理回收。

上传者旁路只用于业务绑定前的预览/提交窗口。FileStore metadata 一旦观察到任一头像、消息、群文件、文档、草稿或任务
...
- 缩略图和小文件可以自动缓存。
- 大文件点击后下载，普通附件消息气泡显示下载进度和取消/重试。Markdown 内嵌资产使用独立的上传
  进度、取消、失败重试与移除入口，状态与恢复边界见[富文本与媒体](../05-clients/rich-content.md)。
- 所有实际网络读取都携带当前会话 access token；图片、音视频加载器也不能例外。
- 下载完成后从本地路径打开，不重复请求。
- 媒体在应用内播放；普通文件可交给系统默认应用。
- 缓存淘汰只删除客户端副本，不改变消息 Attachment。

聊天富资产上传由持久草稿和上传队列持有稳定 identity，以及经过长度/摘要校验的私有源副本；断线和
重启后的重试复用它们，明确过期才建立新 identity。文档和消息编辑的导入仍只在当前进程恢复，普通
独立文件发送沿用自己的发送状态；不能把 transport 的精确重放能力等同于所有 UI 上传都具备跨进程恢复。
本机源文件与消息 outbox 的交接见[客户端架构](../03-architecture/client-and-sdk.md#210-草稿与平台生命周期)。
