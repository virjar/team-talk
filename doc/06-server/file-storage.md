...
附件的反向引用来自当前 User/GroupChat 头像列、消息索引、群文件版本表、文档修订资产表和私有聊天草稿资产表。当前群头像只允许该群现有成员读取；其引用同时进入总附件保留图，避免下载可见却被 GC 当作孤儿回收。消息引用随消息原子写入；群文件引用随文件版本、
...
上传响应成功不等于形成业务引用。未进入当前用户/群头像、消息、活动群文件或活动文档修订的对象从 `uploadedAt` 起持有默认 168 小时
