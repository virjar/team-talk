...
- 备注属于关系所有者，不更新对方 User；`setRemark` 在同一事务更新关系并记录本人专属的
  `CONTACT_UPDATED`，空白备注清为空值，最多 100 字。该事件属于初始协议 0.0，当前设备均可实时收敛；
  不伪造好友接受事件来表达备注变化。
...
个 UTF-16 code unit（Kotlin `String.length`）且不能包含 NUL。这不是 UTF-8 字节数，也不是可见字形数：
同样达到这个长度的纯 ASCII、中文、补充平面 emoji 正文分别约为 1 MB、3 MB、2 MB；合法正文的
UTF-8 上界为 3,000,000 字节。二进制正文解码前另有 4,000,000 字节字段上限，完整 INVOKE 请求体
上限为 4 MiB（包含资产描述符及其他命令字段）。正文通过 TCP RPC 保存，管理 HTTP JSON 的 1 MiB
限制不参与这条链路。

创建、更新及恢复历史为新版本共用服务端正文校验，在创建指纹、绑定暂存附件和写事务之前拒绝超限。
共享 `DocumentPolicy` 信封也用于当前正文、历史修订及本地投影读取；未来若量测后决定收紧写入容量，
须保留旧正文的读取能力，不能直接降低共享信封上限。当前不另设更小的正文 byte cap，也不把现有单篇
正文上限或历史分页当作总修订容量/保留策略。

为保证所有客户端都能有界解析和渲染，正文同时限制为 20,000 个物理行、
...
`TaskService` 通过 `TaskRepository` 与 `PgUnitOfWork` 管理独立任务。创建者和当前单一执行人保持原有
读写边界；群/组织关联不改变归属。显式 `shareToGroup` 的群任务另允许当前群成员只读详情与历史，
旧任务缺省私有，成员不能因此修改任务。普通任务读取与新群查询分别保留对应协议能力。

旧任务、扩展详情/延期和系列启停复用同一可靠收据机制：先识别 actor 作用域的不可变命令，再锁任务
验证当前权限与 expectedRevision，任务、元数据、审计、收据和 TASK_CHANGED 同事务提交。通知覆盖
变更前后读者，失去读权者收到 REVOKED；精确重放不再执行业务，无当前读权时只确认命令。身份冲突
或旧 revision 返回 409，命令统一保留 7 天、每 actor 最多 1,024 条窗口收据，过期 410、满额 429。

详情扩展单独保存格式、开始时间、群共享、文档引用、附件、指标与系列归属，保留已发行 WorkTask
布局。新增文档引用经既有对象解析器规范化，打开时仍校验原文档权利；新附件必须是操作者本人上传的
受管暂存对象，绑定后进入业务引用保留与下载授权链路。群读权不扩大到被引用文档。
...
避免同毫秒或时钟回退复用本机提醒收据；没有固定间隔重复催办，也不依赖进程内计时器恢复。

聊天引用由 `TaskRefResolver` 使用当前任务读入口构造冻结预览，引用不授予读权；既有 `TASK_REF(18)`
与 `OfficeRefBody` 布局不变。协议与旧服务器回退见[待办协议](../04-protocol/rpc-and-events.md#待办任务)。
...
`ServiceAccountMessages` 承载服务号的官方主动触达。欢迎语在客户端拉起（或重入）
`sys_service` 系统会话时按稳定消息身份 `sys-welcome-v1` 自愈补发：已发送过则跳过，
发送中断在下次进入会话时重试，不依赖进程存活。模板由管理台覆盖式更新，只影响之后
首次收到欢迎语的用户，不重发历史欢迎语。

管理员广播从管理台发起：台账先落 PostgreSQL（`service_broadcasts`），随后按 uid 分页
为每个用户确保系统会话、写入待回复记录并立即派发。待回复记录复用服务号回复的持久
发件箱——进程中断后已写记录由启动恢复排空，未扫描的尾部广播在启动时自动续跑；
同一广播重复执行对每个用户按 `sys-bcast-<broadcastId>` 幂等，只补齐未覆盖用户，
不产生重复消息。进度（覆盖/失败计数）按页回写台账，管理台可随时重新推送。
