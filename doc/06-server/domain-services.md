...
`TaskService` 通过 `TaskRepository` 与 `PgUnitOfWork` 管理独立任务。创建者和当前单一执行人可读任务及
审计；创建者可编辑、改派和取消，二者可在待处理、进行中、已完成之间切换，取消后的重开仍只属于
创建者。群或组织只是上下文，新增或改变关联时验证操作者当前可见性，不继承成员权限或改变任务归属。

创建、编辑和状态变更共用不可变 `TaskCommand`。事务先按 uid 排序锁定操作者及请求中的执行人，再
识别 actor 作用域的精确收据，随后锁任务并验证当前权限、`expectedRevision` 与目标状态；任务、审计、
命令收据以及参与者的 `TASK_CHANGED` 在同一事务提交。旧执行人失去读权时收到 `REVOKED`。精确重放
只确认已提交命令，当前无权读取时返回空任务投影；同 ID 改写意图或陈旧修订返回 409。命令使用统一
7 天有效期和每 actor 1,024 条窗口收据上限，过期返回 410，窗口满返回 429。

`task-due-reminders` 维护任务从 PostgreSQL 读取到期候选，再逐任务加锁复验当前状态与截止时间。
提醒标记与给当前执行人的 `TASK_DUE` 原子提交；重启继续扫描，不依赖内存计时器。完成或取消撤销
提醒计划，改派、改截止或重开重新计时；仅改标题保留当前提醒身份。存储保留独立的提醒时间高水位，
即使系统时钟回退或同毫秒重新到期，也不复用旧提醒身份。任务列表按不可变创建时间和 ID 分页，
审计按 revision 分页，每页仍使用当前参与者条件。

聊天引用由 `TaskRefResolver` 调用相同任务读入口构造权威预览；`TASK_REF(18)` 是 minor 2 的新增类型，
