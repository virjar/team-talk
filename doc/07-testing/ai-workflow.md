...
`GroupDetail` / `InviteMembers` / `InviteLinks` 是主窗口聊天右侧覆盖式检查器（入口 `chat.settings`、容器 `chat.inspector`、外部关闭区 `chat.inspector.dismissArea`、关闭按钮 `chat.inspector.close`）。打开后聊天语义仍应存在，用于防止回归为“替换聊天页”；点击外部关闭区后检查器语义应消失、聊天语义应保留。`UserProfile` 是主窗口模态弹窗（`profile.dialog`）；从 `SearchUsers` 点击用户会先关闭任务窗口，再回到主窗口显示资料弹窗。两者都使用默认 `window='main'`。
