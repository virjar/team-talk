...
> `gradle/deployment.json` 指定的真实部署是业务功能的主验收环境。协议级 E2E 由 `:server:acceptanceTest` 执行；
...
| 远程业务 E2E | 验证真实部署、数据库、文件端点与多会话 | `:server:acceptanceTest` | 认证、好友、群聊、消息、文件/图片/语音、编辑/撤回/转发 |
| 真实客户端 | 验证用户可见行为 | `:desktop:run` / Android Debug APK | UI 状态、动画、下载、窗口、截图证据 |

新增业务功能不应只在本地 fake/embedded 环境中完成验收。标准顺序是：

```text
本地安全网 → 部署服务端 → :server:acceptanceTest → 真实客户端语义操作 → 截图/语义树证据
```

本地已存在的重集成测试作为回归资产保留，但新的跨服务业务用例应优先写入 `RemoteAcceptanceTest`。
