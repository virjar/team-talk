...
产品名称统一为“待办任务”；以下为后续产品需求，不表示现有客户端或协议已支持。实施切片见
[TODO-01](../10-reference/roadmap.md#todo-01--待办任务)。现有 `WorkTask`、任务命令与持久提醒继续作为基础，
已有任务、消息引用和历史数据保留，不为产品命名重编已有 wire 编号。

- **群聊悬浮卡片**：关联群存在未完成待办时，在聊天输入区域上方持续显示明确卡片，包含未完成数量、
  近期截止/逾期状态和打开待办列表的入口；完成或取消后实时收敛。它是群聊固定状态区域，不能用一条
  会被聊天滚走的 `TaskRefBody` 消息替代。当前群关联不授予任务读权，后续须明确群成员可见的摘要范围，
  由服务端提供相应查询；不能直接把创建者/执行人的私人任务详情广播给全群。
- **执行人横幅**：当前执行人有已开始且未完成的待办时，在主要界面持续呈现入口，区分待处理、临近
  截止和逾期；不能只在任务页面显示提醒。查看或关闭系统通知不等于完成任务，改派、完成和取消应使
  原执行人的横幅及时消失。摘要和总数来自完整的服务端条件查询，不从已加载的某一页推断。
- **周期待办与强提醒**：支持按明确时区配置循环规则、每期开始和截止时间；例如每周一开始、当天下班
  前完成统计工作，“下班前”在配置中落实为具体时间。每期生成独立、可追踪的待办实例，以系列和期次
  唯一身份防止重启/重试重复生成。从本期开始向目标执行人强提醒，持续显示未完成状态，到期转为逾期；
  提醒时点与频率由服务端统一计划，双端沿现有持久事件收敛。应在实现时明确漏期处理、重复催办间隔、
  上期未完成是否与新一期并列，以及停用/修改规则对未来期次的影响，不循环重置同一个任务抹掉历史。
- **文档与附件**：群作为任务上下文，文档引用及上传附件作为任务的描述材料，可以同时存在。文档引用
  保留原文档身份，打开时验证原文档访问；文件附件进入受管上传、引用和清理流程，具有明确任务归属。
  任务可读不自动赋予其他文档的权限，附件也不能仅靠在描述里拼接文件路径开放下载。改派后不可访问的
  材料应给出明确提示，上传失败或离线待提交不能伪装成已经保存。
- **Markdown 描述**：描述支持 Markdown 编辑与富文本预览，复用现有解析、渲染和附件能力；保留旧任务
  文字，明确旧纯文本中的 Markdown 符号如何显示。无需把简单待办编辑器扩展为另一套企业文档工作区。
- **有理由的延期与指标**：执行人可提交延期，必须填写原因和新的截止时间；保存原承诺截止时间，逐次
  追加操作者、操作时间、原/新截止时间及原因。延期作为可靠命令，重放只形成一次变更和审计，冲突可
  明确处理。展示当前最终截止时间、最近延期操作时间、累计延期次数和处理耗时；统计须明确开始/完成
  时点、重开后的计算方式、取消是否排除及改派后的归属。周期实例分别参与统计，历史缺失的延期事实
  标为未知，不能用当前 `updatedAt` 反推。延期不会改写最初承诺的截止时间，也不会自动完成任务。

验收至少包含每周一开始/下班截止的一期完整流程、服务器停机跨过开始时间后的恢复、同一期不重复生成、
完成与改派后的双端卡片/横幅收敛、文档无权和附件上传失败、Markdown 保存重开，以及多次延期、命令
重试和重开后的历史与指标。群摘要可见范围、周期补期/催办策略和处理耗时口径在实现前形成明确产品规则。
