...
服务端为六类 mutation（创建目录、创建文件、追加版本、重命名、移动、删除）都在同一业务事务中保存
认证操作者、操作类型和规范不可变 payload 指纹的收据。相同标识搭配不同名称、位置、Attachment 或
`expectedRevision` 会冲突，不能重复制造条目、版本或变更。创建与追加版本的重放仍核对具体条目/版本事实；
重命名、移动和删除在 wire 上只返回 `Unit`，因此只有精确命中
`commandId + actor + kind + fingerprint` 的收据才可作为终态 ACK。这是无结果载荷 RPC 的窄例外，不表示
条目当前仍存在，也不为新操作提供准入。移动的目标父目录不能是条目自身或其子孙目录；目标目录已有
同名活动条目时拒绝，移动者须先改名再移。

群消息中的非图片附件在服务端提交时自动归档到群空间固定的“聊天文件”文件夹（重名自动加序号后缀，
归档条目归因于消息发送者）；图片不归档。归档是 best-effort：配额满或成员资格变化等失败只跳过并记
日志，绝不影响消息发送。归档条目与消息引用同一物理附件、引用独立计数——从群空间删除条目不影响
聊天消息中的下载，撤回/删除消息也不影响空间副本。消息引用与群文件引用仍是两个独立事实源；消息
可以引用 GroupFileEntry，权威名称、版本和权限仍由群文件领域维护。
...
`WorkTask` 是以稳定 `taskId` 标识的独立办公对象，`creatorUid` 不可变，`assigneeUid` 是唯一当前执行人。
任务、操作审计与命令收据使用独立身份；`expectedRevision` 控制修改冲突，持久事件驱动客户端投影。
`TaskDetails` 承载描述格式、群共享、材料和延期指标；重复系列生成独立期次，不把某次任务当作模板。

群与组织提供上下文，任务成员身份和显式群共享分别裁决权限。`TaskRefBody` 保存聊天引用与冻结预览，
不授予接收方任务访问权。具体权限、日历重复、提醒和统计口径集中见[待办任务](tasks.md)。
