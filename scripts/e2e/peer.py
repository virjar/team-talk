...
# 冷启动时 Gradle 需先编译 server 测试源集，2 分钟会将正常编译误报为对端失败。
TEST_PEER_TIMEOUT_SECONDS = 10 * 60
...
class TestPeer:
    ...
    所有方法执行 ./gradlew :server:server:test --tests TestPeer.<method>，
    ...
    def _run(self, method, extra_props=None):
        ...
        """
        cmd = [
            os.path.join(self.project_root, "gradlew"), ":server:server:test",
            "--tests", f"com.virjar.tk.server.e2e.TestPeer.{method}",
        ...
        result = subprocess.run(
            ...
            capture_output=True, text=True, timeout=TEST_PEER_TIMEOUT_SECONDS,
        )
    ...
    def _read_xml_stdout(self, method):
        ...
        xml_dir = os.path.join(
            self.project_root, "server", "server", "build", "test-results", "test"
        )
