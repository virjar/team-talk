...
前置：先 ./gradlew :client:desktop:run 启动 Desktop（内置测试 HTTP 服务）。
"""
import json
import os
import time
...
class DesktopClient:
    ...
    def activate_window(self, window=None):
        """请求操作系统将目标 Desktop 窗口置为真实前台窗口。

        用于验收受 ``Window.isActive`` 保护的用户行为；该方法不改写业务前台状态。
        """
        params = {"window": window} if window else None
        code, body = self._req("/window-activate", params, method="POST")
        if code != 200:
            return False
        try:
            return bool(json.loads(body).get("active"))
        except Exception:
            return False
    ...
    def find_test_tag(self, test_tag, window=None):
        """Query a stable testTag through the server's merged+unmerged lookup."""
        params = {"testTag": test_tag}
        if window:
            params["window"] = window
        code, body = self._req("/find", params)
        if code != 200:
            return None
        try:
    ...
    def wait_for_test_tag(self, test_tag, timeout=10, interval=0.25, window=None):
        """Wait for a stable testTag, including a tag nested below a merged clickable row."""
        deadline = time.time() + timeout
        while time.time() < deadline:
            found = self.find_test_tag(test_tag, window)
