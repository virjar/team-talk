...
/** Android 文件附件下载控制器：cacheDir/downloads 缓存 + 气泡进度动画数据源。 */
class AndroidFileDownloadController(
    ...
    private val downloadsDir = File(appContext.cacheDir, "downloads")
    ...
    private suspend fun downloadInternal(attachment: Attachment, openWhenDone: Boolean) {
        ...
        try {
            ...
            if (shouldOpen) openCached(target, attachment.contentType)
        } catch (e: Exception) {
    ...
    private fun cachedFile(attachment: Attachment): File {
        ...
        return File(downloadsDir, "$key-$leaf")
    }
}
