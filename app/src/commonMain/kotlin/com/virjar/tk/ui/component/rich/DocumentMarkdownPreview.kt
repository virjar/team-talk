...
internal fun DocumentMarkdownPreview(
    ...
    val blocks = remember(markdown) { DocumentMarkdownBlockCodec.parse(markdown) }
    LazyColumn(
        ...
        itemsIndexed(blocks, key = { _, block -> block.key }) { _, block ->
            Box(Modifier.fillMaxWidth().widthIn(max = 920.dp)) {
                DocumentMarkdownBlockPreview(block, onUrlClick)
            }
        }
    }
}
        ...
        is DocumentQuoteBlock -> Surface(
            ...
            Row(Modifier.padding(14.dp)) {
                ...
                Column(
                    ...
                    DocumentMarkdownBlockCodec.parse(block.innerMarkdown).forEach { nested ->
                        DocumentMarkdownBlockPreview(nested, onUrlClick)
                    }
                }
            }
        }
        is DocumentCodeFenceBlock -> DocumentCodePreview(block)
        is DocumentGfmTableBlock -> DocumentTablePreview(block, onUrlClick)
