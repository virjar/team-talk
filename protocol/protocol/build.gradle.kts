...
val frozenProtocolContract = release.ProtocolContractPolicy.latest(rootDir)
ksp {
    ...
    arg("teamtalk.publishedProtocolBaseline", frozenProtocolContract.wireBaseline.absolutePath)
...
tasks.register("prepareProtocolContract") {
    group = "release preparation"
    description = "Freeze the reviewed wire contract for an explicitly authorized distribution without changing the product version"
    dependsOn("verifyProtocolBaseline")
    doLast {
        release.ProtocolContractPolicy.prepare(rootDir, protocolMajor, protocolMinor, minimumProtocolMinor)
    }
}
...
tasks.matching { it.name == "kspCommonMainKotlinMetadata" }.configureEach {
    ...
    inputs.file(frozenProtocolContract.wireBaseline).withPropertyName("publishedWireBaseline")
}
