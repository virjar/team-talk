...
data class RichTextBody(
    ...
    /** [markdown] 中 `teamtalk-asset://` 引用的服务端权威伴随清单。 */
    ...
    companion object : IProtoReader<RichTextBody> {
        ...
        override fun readFrom(buf: PacketBuffer): RichTextBody {
            ...
            val assetCount = buf.readCollectionSize(
                ...
                fieldName = "rich-text embedded assets",
            )
            val assets = MarkdownAssetPolicy.canonicalize(
                markdown,
                List(assetCount) { EmbeddedAsset.readFrom(buf) },
            )
            return RichTextBody(markdown, mentions, plainText, assets)
        }
    }
}
...
private data class ProtectedMarkdownCode(
    ...
    fun restore(value: String): String {
        ...
        literals.forEachIndexed { index, literal ->
            restored = restored.replace("$delimiter$index$delimiter", literal)
        }
        return restored
    }
}
...
/**
 * 在周围 Markdown 被化简为纯文本期间，保护 CommonMark 代码内容。
 * 分隔符由源文本中不存在的 NUL 串构成。它对所有剥离 Markdown 的正则都保持惰性，
 * 即使调用方自行提供了 NUL（尚未被准入拦截）也能正常工作。
 */
private fun protectMarkdownCodeLiterals(markdown: String): ProtectedMarkdownCode {
    val ranges = MarkdownAssetPolicy.codeLiterals(markdown)
    if (ranges.isEmpty()) return ProtectedMarkdownCode(markdown, emptyList())
    var delimiter = "\u0000"
    while (delimiter in markdown) delimiter += '\u0000'
    val output = StringBuilder(markdown.length)
    val literals = ArrayList<String>(ranges.size)
    var cursor = 0
    ranges.forEach { range ->
        output.append(markdown, cursor, range.startOffset)
        output.append(delimiter).append(literals.size).append(delimiter)
        literals += markdown.substring(range.contentStart, range.contentEndExclusive)
        cursor = range.endOffsetExclusive
    }
    output.append(markdown, cursor, markdown.length)
    return ProtectedMarkdownCode(output.toString(), literals, delimiter)
}
