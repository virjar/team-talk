...
object MessageBodyPolicy {
    ...
    fun canonicalize(message: Message): Message {
        requireNoNul(message.chatId, "chatId")
        requireNoNul(message.clientMsgId, "clientMsgId")
        requireNoNul(message.senderUid, "senderUid")
        ...
        val canonicalBody = when (body) {
            is RichTextBody -> buildRichTextBody(validateMarkdown(body.markdown), body.assets).also {
            ...
            is EditBody -> body.copy(
                ...
                newContent = MarkdownAssetPolicy.requireNoInternalReferences(
                    validateMarkdown(body.newContent),
                    "编辑正文",
                ),
            )
    ...
    fun validateMarkdown(markdown: String): String {
        requireNoNul(markdown, "Markdown 正文")
    ...
    private fun validateReply(body: ReplyBody): ReplyBody {
        ...
        val content = validateMarkdown(body.content)
        return body.copy(
            content = content,
            assets = MarkdownAssetPolicy.canonicalize(content, body.assets),
        )
    }
    ...
    private fun validateAttachment(attachment: com.virjar.tk.protocol.model.Attachment) {
        requireNoNul(attachment.path, "附件路径")
        requireNoNul(attachment.name, "附件名称")
        requireNoNul(attachment.contentType, "附件类型")
    ...
    private fun validateContactCard(body: CardBody): CardBody {
        ...
        requireNoNul(body.targetName, "联系人名称")
        require(body.targetName.isNotBlank() && body.targetName.length <= MAX_DISPLAY_NAME_LENGTH) {
            "联系人名称非法或过长"
        }
        val avatar = body.targetAvatar?.let(com.virjar.tk.protocol.model.UserAvatarPolicy::requireCanonical)
        return if (avatar == body.targetAvatar) body else body.copy(targetAvatar = avatar)
    }
    ...
    private fun validateReaction(body: ReactionBody): ReactionBody {
        ...
        requireReactionEmoji(body.emoji, "Reaction 表情")
        require(body.action == 0 || body.action == 1) { "Reaction action 非法" }
        return body
    }

    /** 引用消息的客户端字段仍做结构校验；title/subtitle 的权威快照由服务端重建。 */
    private fun validateOfficeRef(body: OfficeRefBody): OfficeRefBody {
        require(body.refType == OfficeRefBody.REF_TYPE_DOCUMENT ||
            body.refType == OfficeRefBody.REF_TYPE_GROUP_FILE) { "office ref 类型非法" }
        requireIdentifier(body.spaceId, "office ref spaceId")
        requireIdentifier(body.targetId, "office ref targetId")
        requireNoNul(body.title, "office ref title")
        require(body.title.isNotBlank() && body.title.length <= MAX_DISPLAY_NAME_LENGTH) {
            "office ref title 非法或过长"
        }
        requireNoNul(body.subtitle, "office ref subtitle")
        require(body.subtitle.length <= MAX_SHORT_TEXT_LENGTH) { "office ref subtitle 过长" }
        return body
    }

    /** ReactionBody 与回应聚合/RPC 共用同一份 emoji 规则：非空、无 NUL、长度受限。 */
    fun requireReactionEmoji(value: String, name: String): String {
        requireNoNul(value, name)
        require(value.isNotBlank() && value.length <= MAX_EMOJI_LENGTH) { "$name 非法或过长" }
        return value
    }
    ...
    private fun requireOptionalLength(value: String?, maximum: Int, name: String) {
        value?.let { requireNoNul(it, name) }
        require(value == null || value.length <= maximum) { "$name 不能超过 $maximum 个字符" }
    }

    /** PostgreSQL text/varchar 无法表示 U+0000；所有跨端消息文本在权威写入前统一拒绝。 */
    private fun requireNoNul(value: String, name: String): String {
        require('\u0000' !in value) { "$name 不能包含 NUL (U+0000)" }
        return value
    }
    ...
    private fun validateInteractiveCard(body: InteractiveCardBody): InteractiveCardBody {
        requireNoNul(body.payloadJson, "交互卡片 JSON")
        ...
        card.title?.let { title ->
            requireNoNul(title, "交互卡片标题")
            require(title.length <= MAX_INTERACTIVE_CARD_TEXT_LENGTH) { "交互卡片标题过长" }
        }
        card.blocks.forEach { block ->
            when (block) {
                is CardBlock.Text -> {
                    requireNoNul(block.text, "交互卡片文本块")
                    require(block.text.length <= MAX_INTERACTIVE_CARD_TEXT_LENGTH) {
                        "交互卡片文本块过长"
                    }
                }
            }
...
fun MessageBody?.plainTextContentOrNull(): String? = when (this) {
    ...
    is ReplyBody -> content.takeIf { it.isNotBlank() }?.let { buildRichTextBody(it, assets).plainText }
