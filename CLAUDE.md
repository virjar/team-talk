...
> **开发阶段，不需要考虑向后兼容**。数据库 schema 变更、本地存储格式调整、协议修改等均不需要做迁移逻辑，直接改即可。等正式发布后再考虑兼容策略。
...
│       ├── kotlin/.../storage/       # 内嵌文件存储（FileStore + FileSystemTier）
...
│       │   ├── ConversationService / DeviceService
...
UserContext ─── 跨平台用户会话核心
  ...
  ├── AppDatabase: 按用户隔离的本地数据库（users/${uid}/teamtalk.db）
  ├── ImageCache: 按用户隔离的图片缓存（users/${uid}/cache/images/）
  └── 6 个 Repository 实例
```
...
| Desktop 用户数据 | `~/.tk/app_default/users/${uid}/`（数据库 + 图片缓存） |
...
4. **合并到 main** — 需经用户确认后方可执行合并操作，**必须使用 `--squash` 方式**将分支压缩为单一提交，避免开发过程中的碎片提交污染 main 历史
