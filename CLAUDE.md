...
> 反例教训：Desktop 测试服务隔离曾尝试用 `-P` 参数 / 动态环境矩阵等方案，
...
- **遇到「要不要加个开关/参数」的决策时，默认不加**。优先用单一部署配置和确定性逻辑
- **不引入环境矩阵 / flavor / 服务器切换 UI**。客户端、部署和远程验收统一读取 `gradle/deployment.json`；fork 通过修改该文件配置自己的私有服务器
...
- **业务验收以真实部署为主**：新增跨客户端/RPC 业务场景优先加入 `RemoteAcceptanceTest`，通过 `:server:acceptanceTest` 执行
...
| 添加服务端 e2e | `server/src/test/.../RemoteAcceptanceTest.kt`，由 `:server:acceptanceTest` 对真实部署执行 |
...
./gradlew :desktop:run                                  # Desktop 连接已配置服务器
...
- **远程 E2E 测试**：`RemoteAcceptanceTest` 连接 `gradle/deployment.json` 指定的服务器，只通过 `./gradlew :server:acceptanceTest` 作为公开入口。测试账号用户名前缀 `e2e-`（≤50 字符）

## CI/CD

- **CI**（`.github/workflows/ci.yml`）：push/PR 自动编译 + 本地确定性测试
- **远程验收**（`.github/workflows/acceptance.yml`）：对已配置的部署执行业务 E2E
