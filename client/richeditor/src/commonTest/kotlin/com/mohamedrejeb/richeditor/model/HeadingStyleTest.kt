...
class HeadingStyleTest {
    ...
    fun defaultsAreFrameworkAgnostic() {
        ...
        assertEquals(TextUnit.Unspecified, HeadingStyle.Normal.defaultParagraphStyle.lineHeight)

        val expectedLineHeights = mapOf(
            HeadingStyle.H1 to 2.5.em,
            HeadingStyle.H2 to 1.875.em,
            HeadingStyle.H3 to 1.4625.em,
            HeadingStyle.H4 to 1.4.em,
            HeadingStyle.H5 to 1.0375.em,
            HeadingStyle.H6 to 0.9375.em,
        )
        expectedLineHeights.forEach { (heading, expectedLineHeight) ->
            val fontSize = heading.defaultSpanStyle.fontSize
            val lineHeight = heading.defaultParagraphStyle.lineHeight
            assertEquals(expectedLineHeight, lineHeight, "$heading line height")
            assertTrue(lineHeight.value >= fontSize.value, "$heading line height must fit its font")
        }
    }
    ...
    @Test
    fun headingLineHeightTracksStyleTransitions() {
        val paragraph = RichParagraph(type = DefaultParagraph()).apply {
            children.add(RichSpan(text = "Wrapped heading", paragraph = this))
        }

        paragraph.applyHeadingStyle(HeadingStyle.H1)
        assertEquals(2.5.em, paragraph.paragraphStyle.lineHeight)

        paragraph.applyHeadingStyle(HeadingStyle.H2)
        assertEquals(1.875.em, paragraph.paragraphStyle.lineHeight)

        paragraph.applyHeadingStyle(HeadingStyle.Normal)
        assertEquals(TextUnit.Unspecified, paragraph.paragraphStyle.lineHeight)
    }
