...
public class RichTextState internal constructor(
     ...
     */
    // [TT] Ordered-list prefixes are user-controlled (for example, "100000. "). Keep this
    // measurement cache bounded so a long-lived editor cannot retain every prefix ever seen.
    internal val startTextWidthCache: MutableMap<String, TextUnit> = linkedMapOf()
    ...
    // [TT] TeamTalk 定制：光标处插入文本（表情/mention 语法插入用）。
    // 上游无公开的定点插入 API（annotatedString 直写绕过段落解析），经 onTextFieldValueChange
    // 走完整内部链路（含撤销历史）。登记：richeditor/FORK.md
    public fun insertAtCaret(text: String) {
        val v = textFieldValue
        val pos = v.selection.min
        val newText = v.text.substring(0, pos) + text + v.text.substring(pos)
        onTextFieldValueChange(
            v.copy(text = newText, selection = TextRange(pos + text.length))
        )
    }

    // [TT] TeamTalk 定制：替换 [start, end) 区间文本（@补全选中替换 / 指令回填用），
    // 保持选区语义与 insertAtCaret 一致。登记：richeditor/FORK.md
    public fun replaceRange(start: Int, end: Int, text: String) {
        val v = textFieldValue
        val safeStart = start.coerceIn(0, v.text.length)
        val safeEnd = end.coerceIn(safeStart, v.text.length)
        val newText = v.text.substring(0, safeStart) + text + v.text.substring(safeEnd)
        onTextFieldValueChange(
            v.copy(text = newText, selection = TextRange(safeStart + text.length))
        )
    }
...
//            }
        ...
        textLayoutResult?.let { textLayoutResult ->
            ...
            paragraphs.forEachIndexed { index, richParagraph ->
                ...
                if (
                    ...
                    recordMeasuredPrefixWidth(paragraphType.startText, distanceSp)
                }
            }
        }
    ...
    internal fun recordMeasuredPrefixWidthForTest(startText: String, width: TextUnit) {
        recordMeasuredPrefixWidth(startText, width)
    }

    // [TT] A tiny FIFO is sufficient here: bullets and the common number widths stay hot, while
    // pathological or very large ordered-list prefixes cannot create an unbounded session cache.
    private fun recordMeasuredPrefixWidth(startText: String, width: TextUnit) {
        if (
            startText !in startTextWidthCache &&
            startTextWidthCache.size >= START_TEXT_WIDTH_CACHE_MAX_ENTRIES
        ) {
            startTextWidthCache.keys.firstOrNull()?.let(startTextWidthCache::remove)
        }
    ...
    public companion object {
        internal const val START_TEXT_WIDTH_CACHE_MAX_ENTRIES: Int = 256
