    ...
    CompositionLocalProvider(LocalClipboard provides richClipboardManager) {
        ...
        BasicTextField(
            ...
            onValueChange = {
                ...
                val previousValue = state.textFieldValue
                state.onTextFieldValueChange(it)
                // [TT] Publish after RichTextState accepts/transforms the edit. Cursor and
                // selection-only BasicTextField callbacks must remain silent.
                if (isUserTextChange(previousValue, state.textFieldValue)) onUserTextChange()
            },
            modifier = modifier
...
// [TT] Kept pure so the selection-only contract is covered without a platform UI harness.
internal fun isUserTextChange(previous: TextFieldValue, current: TextFieldValue): Boolean =
