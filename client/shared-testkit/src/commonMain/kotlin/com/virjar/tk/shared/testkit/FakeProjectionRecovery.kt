...
internal fun applyFakeHistoryProjection(
    ...
    synchronized(messagesByChat) {
        ...
        val base = if (resetResidentWindow) {
            current.filter {
                it.clientMsgId in preserveClientMsgIds ||
                    it.clientMsgId in retainResidentClientMsgIds ||
                    it.serverSeq <= 0L ||
                    (pageMaxSeq != null && it.serverSeq > pageMaxSeq)
            }
        } else {
            current
        }
...
/** 镜像 epoch-3 对没有持久化可靠发件箱的乐观投影所做的启动修复。 */
