...
internal fun DocumentMarkdownPreview(
    ...
    val previewNodes = remember(markdown, assets) {
        DocumentMarkdownPreviewPlanner.plan(
            blocks = DocumentMarkdownBlockCodec.parse(markdown, assets),
            assets = assets,
        )
    }
    SelectionContainer {
...
private fun DocumentMarkdownBlockPreview(
    ...
    when (block) {
        ...
        is DocumentEmbeddedImageBlock -> DocumentEmbeddedAssetPreview(
            block = block,
...
@Composable
private fun DocumentEmbeddedAssetPreview(
    ...
    Surface(
        ...
        Column(Modifier.padding(14.dp)) {
            Text(
                if (presentation == EmbeddedAssetPresentation.IMAGE) "图片" else "文件",
