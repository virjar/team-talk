...
/** 移除一个资源放置，同时保持源码编辑器定向选区稳定。 */
internal fun TextFieldValue.removeEmbeddedAssetReferences(assetId: String): TextFieldValue {
    val references = embeddedAssetReferencesForRemoval(text, assetId)
    if (references.isEmpty()) return this
    return copy(
        text = removeEmbeddedAssetReferenceRanges(text, references),
        selection = TextRange(
            start = mapOffsetAfterEmbeddedAssetRemoval(selection.start, references),
            end = mapOffsetAfterEmbeddedAssetRemoval(selection.end, references),
        ),
        composition = null,
    )
}

private fun mapOffsetAfterEmbeddedAssetRemoval(
    offset: Int,
    ...
    references.forEach { reference ->
        when {
            offset <= reference.startOffset -> return offset - removedBefore
            offset < reference.endOffsetExclusive -> return reference.startOffset - removedBefore
            else -> removedBefore += reference.endOffsetExclusive - reference.startOffset
        }
    }
