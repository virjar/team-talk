...
internal object XiaomiPushSdk {
    val available = Build.MANUFACTURER.equals("Xiaomi", ignoreCase = true)
...
class XiaomiPushReceiver : PushMessageReceiver() {
    override fun onReceiveRegisterResult(context: Context, message: MiPushCommandMessage) {
        ...
        if (message.resultCode == 0L) {
            XiaomiPushSdk.registrationFailureCode.value = null
            XiaomiPushSdk.registrationId.value = message.commandArguments?.firstOrNull().orEmpty()
        } else {
            XiaomiPushSdk.registrationFailureCode.value = message.resultCode
