...
internal actual fun canonicalDeploymentTcpHost(host: String): String = canonicalJvmDeploymentTcpHost(host)

internal actual fun deploymentSha256Hex(value: String): String = MessageDigest.getInstance("SHA-256")
    .digest(value.toByteArray(Charsets.UTF_8))
    .joinToString("") { byte -> "%02x".format(byte.toInt() and 0xff) }

private fun canonicalJvmDeploymentTcpHost(host: String): String {
    ...
    if (':' in unwrapped) {
        ...
        val address = try {
            InetAddress.getByName(unwrapped)
        } catch (failure: Exception) {
            throw IllegalArgumentException("TCP host is not a valid IPv6 literal", failure)
        }
    ...
    val canonical = try {
        ...
        throw IllegalArgumentException("TCP host is invalid", failure)
    }
    require(canonical.length <= 253) { "TCP host is too long" }
    return canonical
}
