...
/** 一个已验证的专用 agent 目录。子目录的创建绝不会修复预先存在的路径。 */
internal class AgentDataDirectory internal constructor(
    ...
    fun ensurePrivateChild(name: String): Path {
        ...
        if (!Files.exists(child, LinkOption.NOFOLLOW_LINKS)) {
            ...
            created = true
            // POSIX 创建属性仍然受进程 umask 约束。这个路径是
            // 我们在已验证的私有根目录中创建的，因此设置其精确 mode
            // 并不等于接管或修复调用方拥有的存储。
...
/**
 * 建立专用数据根目录，且从不接管或 chmod 任意目录。
 * 每个已存在的路径组件都用 NOFOLLOW 检查；只允许创建一个缺失的叶子。
 */
internal object AgentDataDirectoryPolicy {
    ...
    /** 安装器最后一步：验证先前显式的准备工作，并且绝不创建任何东西。 */
