 ...
 * 捕获规则：
 * - [AppError] 直接透传（已类型化）
 * - [TransportUnavailableException] → [AppError.Network]
 * - [kotlinx.coroutines.CancellationException] 保持取消语义，不转成业务失败
 * - 其他 [IllegalStateException] 是编程/生命周期缺陷，不依赖异常文案猜测网络状态
 * - 其他普通 [Exception] → [AppError.Unknown]
 * - `Error`/VM fatal failure 保持传播，绝不伪装成可恢复 UI 错误
 *
