...
class ReleaseMetadata(private val root: File) {
    ...
    fun verify(version: ReleaseVersion, expectedRevision: String, requireClean: Boolean = true): String {
        verifySource(version, expectedRevision, requireClean)
    ...
    /** Private delivery uses the same committed source boundary without claiming public release notes. */
    fun verifySource(version: ReleaseVersion, expectedRevision: String, requireClean: Boolean = true) {
        ...
        if (requireClean) require(git("status", "--porcelain", "--untracked-files=normal").isBlank()) {
            "Commit source, root configuration and compatibility records before distribution"
        }
    }
    ...
    fun verifyFrozenHistorySince(baseRevision: String) {
        ...
        val rewritten = git("diff", "--name-only", "--diff-filter=DMRT", baseRevision, "HEAD", "--",
            "protocol/protocol/releases", "protocol/protocol/contracts")
