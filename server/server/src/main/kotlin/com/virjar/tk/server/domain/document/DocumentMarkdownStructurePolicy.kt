...
internal object DocumentMarkdownStructurePolicy {
    ...
    fun validate(
        ...
        markdown.lineSequence().forEach { physicalLine ->
            ...
            if (openFence != null) {
                ...
                // 引用/列表中的围栏代码块在物理行不再满足其开启容器时结束。把这一行当作
                // 普通 Markdown 重新处理；否则一个不配对的围栏可能压制之后的所有预算。
                fence = null
            }
            ...
            // 一个 Markdown 顶层节点需要至少一个非空的物理源码行。统计每一行这样的行是
            // 刻意保守的做法，并且与构建 AST 不同，它在一个不可信文档到达任何客户端解析器
            // 之前就约束了分配。围栏代码正文行已在上面排除，因为整个围栏是一个可渲染的
            // 文档块。
            ...
            // 每个物理行至多解析一次管道形状。该摘要大小恒定，并可在这一行终止一个表格
            // 并成为下一个可能表头时复用。
    ...
    private fun String.tableLineOrNull(): TableLine? {
            ...
            when {
                ...
                char.isWhitespace() -> if (hasDelimiterMarker) pendingTrimmedWhitespace = true
                    ...
                    if (pendingTrimmedWhitespace) {
                        // `trim()` 只移除单元格边缘的空白。因此标记字符之间的其他 Unicode
                        // 空白会使该分隔符无效。
        ...
        for (index in trimmedStart until trimmedEnd) {
            ...
            // 前导管道之前的空切片不是 GFM 单元格。之后的每个切片，包括相邻管道之间的
            // 空切片，都是真实单元格，必须计数。
