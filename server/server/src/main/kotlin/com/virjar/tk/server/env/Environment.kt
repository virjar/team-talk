 ...
 *
     ...
     * ├── dataset-id          # 与 PostgreSQL schema_metadata 共享的数据集身份
     * ├── rocksdb/            # RocksDB 消息存储
     * ├── lucene-index/       # Lucene 全文索引
     * ├── client-telemetry-index/ # Lucene 七天可丢遥测事实；损坏时清空
     * ├── connection-trace-index/ # Lucene 七天可丢服务端连接轨迹；损坏时清空
     * ├── file-store/
     * │   ├── rocksdb         # 文件存储索引
     * │   ├── files           # 大文件存储
     * │   └── tmp             # 临时文件
...
object Environment {
    ...
    /** 普通附件 FileStore 的全局持久容量硬上限，默认 10 GiB。 */
    val fileStoreQuotaBytes: Long = (
            System.getProperty("teamtalk.fileStore.quotaBytes")
                ?: System.getenv("TEAMTALK_FILE_STORE_QUOTA_BYTES")
    ...
    /** 客户端遥测独立 Lucene 七天事实存储；与消息索引隔离，损坏时安全清空。 */
    val clientTelemetryIndexDir: File = ensureDir(File(dataRoot, "client-telemetry-index"))

    /** 服务端连接诊断的独立有界 Lucene 存储；不得与消息或客户端遥测索引共用目录。 */
    val connectionTraceIndexDir: File = ensureDir(File(dataRoot, "connection-trace-index"))
