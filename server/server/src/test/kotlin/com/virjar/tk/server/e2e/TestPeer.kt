...
class TestPeer {
    ...
    fun acceptLatestFriend() = runBlocking {
        ...
        val applies = ProtoCodec.decodeList(com.virjar.tk.protocol.model.ContactApply, listPayload)
    ...
    fun createPersonalChat() = runBlocking {
        ...
        val chatId = if (resp.status == 0 && payload != null) {
            ProtoCodec.decode(com.virjar.tk.protocol.model.Chat, payload).chatId
        } else ""
    ...
    fun setupGroupAndPrivate() = runBlocking {
        ...
        val groupChatId = if (groupResp.status == 0 && groupPayload != null) {
            ProtoCodec.decode(com.virjar.tk.protocol.model.Chat, groupPayload).chatId
        } else ""
        ...
        val pcChatId = if (pcResp.status == 0 && pcPayload != null) {
            ProtoCodec.decode(com.virjar.tk.protocol.model.Chat, pcPayload).chatId
        } else ""
    ...
    fun recvCheck() = runBlocking {
        ...
        val session = RemoteAcceptanceSupport.loginUser(username, "password123")
        // 收齐有界会话快照页（conversation.listPage，非 ChatMethod）
        val conversations = mutableListOf<Conversation>()
        val seenCursors = hashSetOf<String>()
        var cursor: String? = null
        while (true) {
            val listResp = session.invoke(
                "conversation",
                ConversationRpcContract.M_LIST_PAGE,
                ProtoCodec.encode(ConversationPageRequest(cursor)),
            )
            val listPayload = listResp.payload
            if (listResp.status != 0 || listPayload == null) {
            ...
            val page = ProtoCodec.decode(ConversationPage, listPayload)
            conversations += page.items
            val next = page.nextCursor ?: break
            if (!seenCursors.add(next)) {
                println("===RECV_CHECK FAILED=== conversation cursor did not advance")
                session.close(); return@runBlocking
            }
            cursor = next
        }
        ...
        val conv = conversations.firstOrNull { c ->
            ...
            membersResp.status == 0 && membersResp.payload?.let { pl ->
                ProtoCodec.decodeList(com.virjar.tk.protocol.model.Member, pl).map { it.uid }.toSet() == setOf(targetUid, session.uid)
            } ?: false
        }
        if (conv == null) {
    ...
    private fun createMinimalPng(): File {
        ...
        DataOutputStream(FileOutputStream(f)).use { dos ->
            dos.write(byteArrayOf(-119, 80, 78, 71, 13, 10, 26, 10)) // PNG 签名
    ...
    private fun createMinimalMp3(): File {
        ...
        FileOutputStream(f).use { fos ->
            ...
            // MPEG 音频帧头（4 字节）：MPEG1 Layer3 128kbps 44100Hz
    ...
    fun sendCardAsB() = runBlocking {
        ...
        val chatId = System.getProperty("peer.arg") ?: return@runBlocking
        val session = RemoteAcceptanceSupport.loginUser(username, "password123")
        val card = com.virjar.tk.protocol.body.CardPayload(
            title = "构建通知",
            blocks = listOf(
                com.virjar.tk.protocol.body.CardBlock.Text("分支 main 构建通过，耗时 3m12s"),
                com.virjar.tk.protocol.body.CardBlock.Text("提交：飞书风格设计系统落地"),
            ),
        )
        val msg = Message(chatId = chatId, clientMsgId = UUID.randomUUID().toString(),
            messageType = MessageType.INTERACTIVE_CARD.code, timestamp = System.currentTimeMillis(),
            senderUid = session.uid, body = com.virjar.tk.protocol.body.InteractiveCardBody.of(card))
        val ack = session.imClient.sendAndWaitAck(msg)
