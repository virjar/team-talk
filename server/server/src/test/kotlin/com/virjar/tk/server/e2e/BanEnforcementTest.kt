    ...
    fun `封禁后的登录与旧凭据重连返回账号封禁终局判定`() {
        TcpE2eEnvironment().use { env ->
            runBlocking {
                ...
                val registered = CompletableDeferred<Pair<String, String>>()
                val c1 = ImClient(onAuthResult = { ok, uid, _, _, refresh, _, _, _ ->
                    if (ok) registered.complete(assertNotNull(uid) to assertNotNull(refresh))
                })
                val (uid, refresh) = try {
                    c1.register(username, "password123", "B", "d1", "T", "127.0.0.1", env.tcpPort)
                ...
                suspend fun observeFailure(start: (ImClient) -> Unit): AuthenticationFailure {
                    val observed = CompletableDeferred<AuthenticationFailure>()
                    val client = ImClient(
                        host = "127.0.0.1", port = env.tcpPort,
                        onAuthenticationFailureObserved = { observed.complete(it) },
                    )
                    return try {
                        start(client)
                ...
                // 错误密码不能得到会触发本地资料删除的封禁结果或账号范围。
                val wrongPassword = observeFailure {
                    it.login(username, "wrong-password", "d2", "B")
                }
                assertEquals(AuthenticationFailureKind.REJECTED, wrongPassword.kind)
                assertNull(wrongPassword.accountUid)
                assertNull(wrongPassword.datasetId)

                val bannedLogin = observeFailure {
                    it.login(username, "password123", "d2", "B")
                }
                assertEquals(AuthenticationFailureKind.ACCOUNT_BANNED, bannedLogin.kind)
                assertEquals(uid, bannedLogin.accountUid)
                assertEquals(env.syncDatasetId, bannedLogin.datasetId)

                // 凭据行已删除，但封禁墓碑必须让旧 refresh 得到同一个权威清理范围。
                val bannedRefresh = observeFailure {
                    it.authenticate(uid, refresh, "d1", "T")
                }
                assertEquals(AuthenticationFailureKind.ACCOUNT_BANNED, bannedRefresh.kind)
                assertEquals(uid, bannedRefresh.accountUid)
                assertEquals(env.syncDatasetId, bannedRefresh.datasetId)
            }
        }
    }
}
