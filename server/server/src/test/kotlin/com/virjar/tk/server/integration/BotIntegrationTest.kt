...
class BotIntegrationTest {
    ...
    fun `bot delivery requires credentials and grant and is idempotent`() = runTest {
        ...
        assertFailsWith<BotRequestException> {
            ctx.botService.deliver(
                ...
                group.chatId,
                "invalid key",
                "x".repeat(com.virjar.tk.server.domain.bot.BotService.MAX_IDEMPOTENCY_KEY_LENGTH + 1),
            )
        }
    ...
    fun `dissolving a group disables owned bot and revokes system bot grant`() = runTest {
        ...
        val owned = ctx.botService.createGroupBotForTest(member, group.chatId, "群内机器人")
    ...
    fun `every group member can create while credentials remain creator scoped`() = runTest {
        ...
        assertFailsWith<BotAuthorizationException> {
            ctx.botService.createGroupBotForTest(creator, personalChat.chatId, "错误的私聊机器人")
        }
        assertEquals(botCountBeforePersonalAttempt, ctx.botService.list().size, "私聊请求不应留下服务账号机器人")

        val created = ctx.botService.createGroupBotForTest(creator, group.chatId, "流水线通知")
        ...
        assertFailsWith<BotAuthorizationException> {
            ctx.botService.rotateGroupBotTokenForTest(member, group.chatId, created.bot.botId)
        }
        assertFailsWith<BotAuthorizationException> {
        ...
        val rotated = ctx.botService.rotateGroupBotTokenForTest(creator, group.chatId, created.bot.botId)
        assertFailsWith<BotAuthenticationException> {
            ctx.botService.deliver(
                created.bot.botId,
                created.webhookToken,
    ...
    fun `group bot quota limits one creator without reserving the group for admins`() = runTest {
        ...
        val owned = (1..com.virjar.tk.server.domain.bot.BotService.MAX_MANAGED_BOTS_PER_CREATOR_IN_GROUP).map { index ->
            ctx.botService.createGroupBotForTest(creator, group.chatId, "创建者机器人-$index")
        }
        assertFailsWith<IllegalArgumentException> {
            ctx.botService.createGroupBotForTest(creator, group.chatId, "超过个人配额")
        }

        val teammateBot = ctx.botService.createGroupBotForTest(teammate, group.chatId, "团队成员机器人")
        assertTrue(teammateBot.bot.createdByMe)

        ctx.botService.removeFromGroup(creator, group.chatId, owned.first().bot.botId)
        val replacement = ctx.botService.createGroupBotForTest(creator, group.chatId, "释放配额后创建")
