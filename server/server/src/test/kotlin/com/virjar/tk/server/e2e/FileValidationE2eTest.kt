...
class FileValidationE2eTest {
    ...
    fun `other users cannot claim an unattached upload but chat members may reuse a referenced file`() {
        TcpE2eEnvironment().use { env ->
            runBlocking {
                val owner = ImBot.register("127.0.0.1", env.tcpPort, "file-owner", TEST_IM_BOT_PASSWORD, testImBotCacheOwner)
                val member = ImBot.register("127.0.0.1", env.tcpPort, "file-member", TEST_IM_BOT_PASSWORD, testImBotCacheOwner)
                try {
    ...
    fun `相对 path 引用真实文件 - 发送成功`() {
        TcpE2eEnvironment().use { env ->
            runBlocking {
                val b = ImBot.register("127.0.0.1", env.tcpPort, "fileval-b", TEST_IM_BOT_PASSWORD, testImBotCacheOwner)
                val a = ImBot.register("127.0.0.1", env.tcpPort, "fileval-a", TEST_IM_BOT_PASSWORD, testImBotCacheOwner)
                try {
    ...
    fun `完整 URL 形态 - 剥前缀后校验成功`() {
        TcpE2eEnvironment().use { env ->
            runBlocking {
                val b = ImBot.register("127.0.0.1", env.tcpPort, "fileval2-b", TEST_IM_BOT_PASSWORD, testImBotCacheOwner)
                val a = ImBot.register("127.0.0.1", env.tcpPort, "fileval2-a", TEST_IM_BOT_PASSWORD, testImBotCacheOwner)
                try {
                    val chatId = a.createPersonalChat(b.uid)
    ...
    fun `不存在的附件 path - 服务端拒绝`() {
        TcpE2eEnvironment().use { env ->
            runBlocking {
                val b = ImBot.register("127.0.0.1", env.tcpPort, "fileval3-b", TEST_IM_BOT_PASSWORD, testImBotCacheOwner)
                val a = ImBot.register("127.0.0.1", env.tcpPort, "fileval3-a", TEST_IM_BOT_PASSWORD, testImBotCacheOwner)
                try {
                    val chatId = a.createPersonalChat(b.uid)
                    ...
                    assertEquals(
                        OutgoingFailureCode.REMOTE_REJECTED.publicMessage,
                        ack.reason,
                        "SDK 不得把服务端自由文本拒绝原因暴露给调用方",
                    )
                } finally { a.shutdown(); b.shutdown() }
            }
        }
    ...
    fun `伪造小文件大小 - 服务端拒绝`() {
        TcpE2eEnvironment().use { env ->
            runBlocking {
                val b = ImBot.register("127.0.0.1", env.tcpPort, "fileval-size-b", TEST_IM_BOT_PASSWORD, testImBotCacheOwner)
                val a = ImBot.register("127.0.0.1", env.tcpPort, "fileval-size-a", TEST_IM_BOT_PASSWORD, testImBotCacheOwner)
                try {
                    val chatId = a.createPersonalChat(b.uid)
    ...
    fun `图片消息假 URL - 同样拦截`() {
        TcpE2eEnvironment().use { env ->
            runBlocking {
                val b = ImBot.register("127.0.0.1", env.tcpPort, "fileval4-b", TEST_IM_BOT_PASSWORD, testImBotCacheOwner)
                val a = ImBot.register("127.0.0.1", env.tcpPort, "fileval4-a", TEST_IM_BOT_PASSWORD, testImBotCacheOwner)
                try {
                    val chatId = a.createPersonalChat(b.uid)
    ...
    fun `缩略图不存在 - 服务端拒绝整条消息`() {
        TcpE2eEnvironment().use { env ->
            runBlocking {
                val b = ImBot.register("127.0.0.1", env.tcpPort, "fileval5-b", TEST_IM_BOT_PASSWORD, testImBotCacheOwner)
                val a = ImBot.register("127.0.0.1", env.tcpPort, "fileval5-a", TEST_IM_BOT_PASSWORD, testImBotCacheOwner)
                try {
                    val chatId = a.createPersonalChat(b.uid)
