...
class ImageThumbE2eTest {
    ...
    fun `带缩略图的图片消息端到端`() {
        TcpE2eEnvironment().use { env ->
            runBlocking {
                val b = ImBot.register("127.0.0.1", env.tcpPort, "imgthumb-b", TEST_IM_BOT_PASSWORD, testImBotCacheOwner)
                try {
                    val a = ImBot.register("127.0.0.1", env.tcpPort, "imgthumb-a", TEST_IM_BOT_PASSWORD, testImBotCacheOwner)
                    try {
