...
import com.virjar.tk.server.domain.bot.BotDeliveryResult
...
class BotWebhookRoutesTest {
    ...
    @Test
    fun `target webhook rejects an oversized structured body before deserialization`() = testApplication {
        val delivery = RecordingBotDelivery()
        application {
            install(ContentNegotiation) { json() }
            routing { targetBoundBotMessageRoutes(delivery) }
        ...
        val response = client.post("/api/v1/groups/chat-path/bots/bot-1/messages") {
            contentType(ContentType.Application.Json)
            header(HttpHeaders.Authorization, "Bearer ttb_secret")
            setBody("""{"markdown":"${"x".repeat(STRUCTURED_HTTP_BODY_MAX_BYTES)}"}""")
        }
    ...
    @Test
    fun `target webhook maps malformed JSON to bad request`() = testApplication {
        val delivery = RecordingBotDelivery()
        application {
            install(ContentNegotiation) { json() }
            routing { targetBoundBotMessageRoutes(delivery) }
