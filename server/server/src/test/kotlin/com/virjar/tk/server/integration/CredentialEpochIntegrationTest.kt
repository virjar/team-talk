...
import com.virjar.tk.server.infra.sync.credentialEpochsDoNotRegress
...
class CredentialEpochIntegrationTest {
    ...
    fun `credentials persist only hashes and diagnostic strings redact secrets`() = runTest {
        ...
        val persistedHashes = transaction(ctx.database) {
        ...
        assertFalse(
            requireNotNull(ctx.userRepo.findInternalByUid(uid)).toString().contains(proof.passwordHashSnapshot),
        )
    ...
    fun `same device password login keeps only latest credential pair`() = runTest {
        ...
        val firstDeviceEpoch = transaction(ctx.database) {
    ...
    @Test
    fun `lost refresh response retries the stable device bearer without extending its lifetime`() = runTest {
        ...
        val originalRefreshLifetime = transaction(ctx.database) {
            Credentials.selectAll()
                .where { Credentials.tokenHash eq sha256(oldRefresh) }
                .single()
                .let { credential ->
                    credential[Credentials.createdAt] to credential[Credentials.expiresAt]
                }
        }
        assertNotNull(ctx.accessTokenValidator.validateAccessToken(oldAccess))

        // 模拟一次已提交但响应丢失的 AUTH：调用方有意忽略这次成功，
        // 并用其持久保存的唯一 refresh bearer 重试。
        val lostResponse = refresh(oldRefresh, "refresh-device")
        val lostResponseAccess = requireNotNull(lostResponse.accessToken)
        val lostResponsePrincipal = assertNotNull(ctx.accessTokenValidator.validateAccessToken(lostResponseAccess))

        assertEquals(0, lostResponse.code)
        assertEquals(oldRefresh, lostResponse.refreshToken)
        assertNull(ctx.accessTokenValidator.validateAccessToken(oldAccess))

        val retried = refresh(oldRefresh, "refresh-device")
        val retriedAccess = requireNotNull(retried.accessToken)
        val retriedPrincipal = assertNotNull(ctx.accessTokenValidator.validateAccessToken(retriedAccess))

        assertEquals(0, retried.code)
        assertEquals(oldRefresh, retried.refreshToken)
        assertTrue(retriedPrincipal.deviceCredentialEpoch > lostResponsePrincipal.deviceCredentialEpoch)
        assertNull(ctx.accessTokenValidator.validateAccessToken(lostResponseAccess))
        transaction(ctx.database) {
            val credentials = Credentials.selectAll().where { Credentials.uid eq uid }.toList()
            assertEquals(2, credentials.size)
            assertEquals(listOf(1, 2), credentials.map { it[Credentials.tokenType] }.sorted())
            val refreshCredential = credentials.single { it[Credentials.tokenType] == 2 }
            assertEquals(sha256(oldRefresh), refreshCredential[Credentials.tokenHash])
            assertEquals(
                originalRefreshLifetime,
                refreshCredential[Credentials.createdAt] to refreshCredential[Credentials.expiresAt],
            )
            assertEquals(retriedPrincipal.deviceCredentialEpoch, refreshCredential[Credentials.deviceCredentialEpoch])
        }
    }

    @Test
    fun `stable refresh accepts its inclusive expiry boundary and rejects the next millisecond`() = runTest {
        val username = uniqueUsername("credential-refresh-expiry")
        val password = "pass123"
        val uid = ctx.registerUser(username, password)
        val initial = login(username, password, "refresh-expiry-device")
        val stableRefresh = requireNotNull(initial.refreshToken)
        val absoluteExpiry = transaction(ctx.database) {
            Credentials.selectAll()
                .where { Credentials.tokenHash eq sha256(stableRefresh) }
                .single()[Credentials.expiresAt]
        }

        val atBoundaryRepository = ExposedCredentialRepository(
            database = ctx.database,
            clock = { absoluteExpiry },
        )
        val boundaryIssued = assertNotNull(
            atBoundaryRepository.refreshCredentials(stableRefresh, device("refresh-expiry-device")),
        )
        assertEquals(stableRefresh, boundaryIssued.refreshToken)

        val afterBoundaryRepository = ExposedCredentialRepository(
            database = ctx.database,
            clock = { Math.addExact(absoluteExpiry, 1L) },
        )
        assertNull(afterBoundaryRepository.refreshCredentials(stableRefresh, device("refresh-expiry-device")))
        assertNotNull(afterBoundaryRepository.validateAccessToken(boundaryIssued.accessToken))
        transaction(ctx.database) {
            val credentials = Credentials.selectAll().where { Credentials.uid eq uid }.toList()
            assertEquals(2, credentials.size)
            assertEquals(
                absoluteExpiry,
                credentials.single { it[Credentials.tokenType] == 2 }[Credentials.expiresAt],
            )
        }
    }

    @Test
    fun `concurrent refresh retries serialize and leave exactly the newest access pair`() = runBlocking {
        val username = uniqueUsername("credential-refresh-concurrent")
        val password = "pass123"
        val uid = ctx.registerUser(username, password)
        val initial = login(username, password, "refresh-concurrent-device")
        val stableRefresh = requireNotNull(initial.refreshToken)
        val firstLocked = CompletableDeferred<Unit>()
        val releaseFirst = CompletableDeferred<Unit>()
        val secondReachedLockBoundary = CompletableDeferred<Unit>()
        val beforeLockCount = AtomicInteger()
        val afterLockCount = AtomicInteger()
        val repository = ExposedCredentialRepository(
            ...
            hooks = object : CredentialRepositoryHooks {
                override suspend fun beforeUserLock(
                    ...
                    if (operation == CredentialMutation.REFRESH && beforeLockCount.incrementAndGet() == 2) {
                        secondReachedLockBoundary.complete(Unit)
                    }
                }
                ...
                override suspend fun afterUserLock(
                    ...
                    if (operation == CredentialMutation.REFRESH && afterLockCount.incrementAndGet() == 1) {
        ...
        coroutineScope {
            val first = async(Dispatchers.IO) {
                repository.refreshCredentials(stableRefresh, device("refresh-concurrent-device"))
            }
            withTimeout(5_000) { firstLocked.await() }
            val second = async(Dispatchers.IO) {
                repository.refreshCredentials(stableRefresh, device("refresh-concurrent-device"))
            }
            withTimeout(5_000) { secondReachedLockBoundary.await() }
            assertFalse(second.isCompleted, "the second refresh must wait for the locked user aggregate")

            releaseFirst.complete(Unit)
            val firstIssued = assertNotNull(withTimeout(5_000) { first.await() })
            val secondIssued = assertNotNull(withTimeout(5_000) { second.await() })

            assertEquals(stableRefresh, firstIssued.refreshToken)
            assertEquals(stableRefresh, secondIssued.refreshToken)
            assertTrue(secondIssued.principal.deviceCredentialEpoch > firstIssued.principal.deviceCredentialEpoch)
            assertNull(repository.validateAccessToken(firstIssued.accessToken))
            assertNotNull(repository.validateAccessToken(secondIssued.accessToken))
            transaction(ctx.database) {
                val credentials = Credentials.selectAll().where { Credentials.uid eq uid }.toList()
                assertEquals(2, credentials.size)
                assertEquals(listOf(1, 2), credentials.map { it[Credentials.tokenType] }.sorted())
                assertEquals(
                    secondIssued.principal.deviceCredentialEpoch,
                    credentials.single { it[Credentials.tokenType] == 2 }[Credentials.deviceCredentialEpoch],
                )
            }
        }
    }
    ...
    fun `ban and unban never resurrect old credentials`() = runTest {
        ...
        val initialRevision = ctx.userService.getProfile(uid).revision
    ...
    fun `admin password reset revokes old epoch and requires new password`() = runTest {
        ...
        val revisionBeforeReset = ctx.userService.getProfile(uid).revision

        ctx.adminService.resetPassword(uid, newPassword)
        assertEquals(revisionBeforeReset, ctx.userService.getProfile(uid).revision)
        ...
        authConnectionGeneration.incrementAndGet().let { generation ->
            ctx.authService.handleAuth(
                AuthRequestPayload(
                    ...
                    deviceFlag = 1,
                    correlationId = "credential-epoch-auth-$generation",
