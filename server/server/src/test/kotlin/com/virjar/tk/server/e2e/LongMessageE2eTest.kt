...
class LongMessageE2eTest {
    ...
    fun `超长文本消息可发送可接收`() {
        TcpE2eEnvironment().use { env ->
            runBlocking {
                val b = ImBot.register("127.0.0.1", env.tcpPort, "longmsg-b", TEST_IM_BOT_PASSWORD, testImBotCacheOwner)
                try {
                    val a = ImBot.register("127.0.0.1", env.tcpPort, "longmsg-a", TEST_IM_BOT_PASSWORD, testImBotCacheOwner)
                    try {
                        ...
                        assertEquals(longText.length, received.body?.let { (it as? com.virjar.tk.protocol.body.RichTextBody)?.markdown?.length })
                    } finally { a.shutdown() }
                } finally { b.shutdown() }
            }
        }
