 ...
 * 所以通过这个配置类，配置在当前上下文缺少WebServerFactory接口实现类时（即缺少内置Servlet容器），加载EmbeddedNettyFactory
...
public class EmbeddedNettyAutoConfiguration {
    ...
    //当前Spring容器中不存在WebServerFactory接口的实例
    public static class EmbeddedNetty {
        //上述条件注解成立的话就会构造EmbeddedNettyFactory这个WebServerFactory
        @Bean
