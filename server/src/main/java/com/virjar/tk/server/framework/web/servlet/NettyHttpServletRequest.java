...
import com.virjar.tk.server.framework.web.core.NettyAsyncContext;
import com.virjar.tk.server.framework.web.core.NettyRequestDispatcher;
...
public class NettyHttpServletRequest implements HttpServletRequest {
    ...
    public NettyHttpServletRequest(ChannelHandlerContext ctx, NettyServletContext servletContext,
    ...
    private void parseCookie() {
        ...
        this.cookies = new jakarta.servlet.http.Cookie[nettyCookies.size()];
        ...
        while (itr.hasNext()) {
            ...
            jakarta.servlet.http.Cookie servletCookie = new jakarta.servlet.http.Cookie(nettyCookie.name(), nettyCookie.value());
//            servletCookie.setMaxAge(Ints.checkedCast(nettyCookie.maxAge()));
    ...
    private String getSessionIdFromCookie() {
        jakarta.servlet.http.Cookie [] cookies = getCookies();
        if (cookies == null) {
            return null;
        }
    ...
    private final java.util.function.Supplier<Pair<String, Integer>> hostAndPort =
            Suppliers.memoize(new Supplier<Pair<String, Integer>>() {
                @Override
                public Pair<String, Integer> get() {
                    String host = request.headers().get(HttpHeaderNames.HOST);
                    if (host == null) {
                        InetSocketAddress addr = (InetSocketAddress) ctx.channel().localAddress();
                        return Pair.of(NetUtil.getHostname(addr), addr.getPort());
                    }
                    List<String> strings = splitter.splitToList(host);
                    return strings.size() == 1 ?
                            Pair.of(host, 80) :
                            Pair.of(strings.get(0), Integer.parseInt(strings.get(1)));
                }
            });
    ...
    public String getServerName() {
        return hostAndPort.get().getKey();
    }

    @Override
    public int getServerPort() {
        return hostAndPort.get().getValue();
    }
