            ...
            runBlocking {
                ...
                try {
                    ...
                    queue = com.virjar.tk.client.SendQueue(
                        ...
                        scope = queueScope,
                        onQueued = { states.add(com.virjar.tk.model.Message.SEND_STATUS_QUEUED) },
                        onSent = { _, _ -> states.add(com.virjar.tk.model.Message.SEND_STATUS_SENT) },
                    ...
                    assertTrue(com.virjar.tk.model.Message.SEND_STATUS_QUEUED in states, "应进入 QUEUED: $states")

                    // 重连（退避 1s 起）→ AUTHENTICATED → 队列唤醒补发
                    withTimeout(20_000) { b.state.first { it == ConnectionState.AUTHENTICATED } }
                    withTimeout(15_000) {
                        while (com.virjar.tk.model.Message.SEND_STATUS_SENT !in states) kotlinx.coroutines.delay(200)
                    }
