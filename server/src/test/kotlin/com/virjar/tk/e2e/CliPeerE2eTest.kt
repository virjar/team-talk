        ...
        TcpE2eEnvironment().use { env ->
            ...
            try {
                ...
                while (System.currentTimeMillis() < deadline) {
                    val line = agent.inputStream.bufferedReader().readLine() ?: break
                    if ("token=" in line) { token = line.substringAfter("token=").trim(); break }
                }
                check(token.isNotBlank()) { "agent 未就绪" }
                ...
                try {
                    ...
                    val received = cli.recvFrom(a.uid, timeoutSec = 15)
                    assertTrue(received != null, "agent 应收到消息")
