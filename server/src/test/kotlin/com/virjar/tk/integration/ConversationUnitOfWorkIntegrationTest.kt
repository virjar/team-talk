...
/** Real PostgreSQL gates for Conversation state + durable-event atomicity. */
class ConversationUnitOfWorkIntegrationTest {
    ...
    fun `lagging peer advances without duplicating unchanged actor event`() = runTest {
        ...
        // Recreate a deliberately fresh peer inbox while preserving active membership. The actor
        // watermark is already current, but this peer now has a real persisted row at watermark 0.
        ctx.conversationService.deleteConversation(sender, chat.chatId)
        ctx.conversationService.ensureConversations(chat.chatId, chat.chatType, listOf(sender))
    ...
    fun `concurrent high and low mark read commits never regress state or receipts`() = runTest {
        ...
        val services = listOf(
            conversationService(ExposedPgUnitOfWork(onEventsCommitted = {})) to highSeq,
            conversationService(ExposedPgUnitOfWork(onEventsCommitted = {})) to lowSeq,
        )
    ...
    fun `conversation repository rejects a foreign transaction handle`() = runTest {
        ...
        assertFailsWith<IllegalStateException> {
            ctx.conversationRepo.setPin(object : PgTransactionContext {}, owner, chat.chatId, true)
        }
    ...
    private fun eventCount(uid: String, type: NotifyType): Long = transaction {
    ...
    private fun conversationVersion(uid: String, chatId: String): Long? = transaction {
    ...
    private fun latestEventPayload(uid: String, type: NotifyType): ByteArray = transaction {
        SyncEvents.selectAll().where {
            (SyncEvents.uid eq uid) and (SyncEvents.eventType eq type.code)
        }.orderBy(SyncEvents.streamSeq, SortOrder.DESC)
            .limit(1)
    ...
    private fun eventPayloadsAfter(uid: String, type: NotifyType, afterSeq: Long): List<ByteArray> = transaction {
        SyncEvents.selectAll().where {
