...
import com.virjar.tk.protocol.codec.authorizeTypingDelivery
...
class AuthoritativeChatAccessIntegrationTest {
    ...
    fun `another node membership removal immediately closes every content read boundary`() = runTest {
        ...
        ctx.groupFileService.createFile(owner, group.chatId, null, "private.txt", attachment)

        // Simulate a second server: this process has a positive member cache entry, while another
        // process commits the revocation and cannot publish a local invalidation callback here.
        assertNotNull(ctx.chatStore.getMember(group.chatId, member))
