...
/**
 * Hard close-return boundary for session-owned [HttpURLConnection] operations.
 *
 * A connection is registered before its first network I/O. [close] seals admission, disconnects
 * every registered connection, and does not return until every registered operation has left.
 * The operation's first-I/O admission is checked again after registration, so an operation which
 * loses that race is disconnected without touching the network.
 */
class HttpConnectionOperationGate(
    ...
    /**
     * Registers an inert, configured connection. [closedFailure] preserves each adapter's public
     * error contract when registration or first-I/O admission loses a race with [close].
     */
    fun register(
    ...
    /**
     * Seals admission, aborts all admitted connections, then joins their operation exits.
     * Concurrent and repeated callers observe the same completed boundary.
     */
    fun close() {
    ...
    /** Non-blocking terminal inspection for an outer lifecycle owner that already joins operations. */
    ...
    private fun closeAsLeader(role: CloseRole.Leader) {
        role.operations.forEach { operation ->
            // Disconnecting the current operation from a callback can acquire a native lock which
            // that same callback already owns. Seal now and let its guaranteed finally disconnect
            // while the reentrant caller receives a hard-boundary exception.
    ...
    inner class Operation internal constructor(
        ...
        suspend fun <T> executeSuspending(
            ...
            try {
                ...
                // A cancelled context may reject withContext before its body starts. That request
                // was still registered and therefore must leave before close can return.
                if (!executionEntered) finish(dispatchFailure)
            }
        }

        /** Best-effort early abort used by cancellation callbacks; operation exit remains joined. */
        fun abort() {
            ensureDisconnected()
        }
        ...
        /** Linearization point immediately before the adapter performs its first network I/O. */
        ...
        /** Exactly one thread disconnects; close and operation cleanup join that same attempt. */
