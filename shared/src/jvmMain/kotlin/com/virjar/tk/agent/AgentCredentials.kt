...
/** Complete on-disk state. Only REGISTER_PENDING may contain [password]. */
...
/** Stable, non-login identity required before the first authentication attempt. */
...
/** Hash-free local login bootstrap store with fail-closed POSIX ownership and atomic persistence. */
object AgentCredentials {
    ...
    /**
     * Persists the exact registration identity before any network I/O. Existing ACTIVE dataDirs
    ...
    /**
     * Persists the server-issued refresh credential synchronously from ImBot's AUTH callback.
     * This runs before synchronization can reach ready. Pending plaintext password is removed in
     * the same atomic replace, and an ACTIVE dataDir can only rotate its exact account token.
     */
    @Synchronized
    ...
    private fun isValidDeviceId(value: String): Boolean =
        value.length in 8..AuthPayloadPolicy.MAX_DEVICE_ID_LENGTH &&
            value != "." && value != ".." &&
