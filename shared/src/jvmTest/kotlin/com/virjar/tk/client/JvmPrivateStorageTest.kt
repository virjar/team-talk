...
class JvmPrivateStorageTest {
    ...
    @Test
    fun `legacy copy rejects any overwide directory or file before publication`() {
        val sourceRoot = Files.createTempDirectory("teamtalk-private-source-")
        val targetRoot = Files.createTempDirectory("teamtalk-private-target-")
        if (Files.getFileAttributeView(sourceRoot, PosixFileAttributeView::class.java) == null) {
            sourceRoot.toFile().deleteRecursively()
            targetRoot.toFile().deleteRecursively()
            return
        }
        try {
            val source = JvmPrivateDataDirectory.openExisting(sourceRoot.toFile())
            val target = JvmPrivateDataDirectory.openExisting(targetRoot.toFile())
            val sourceFile = source.preparePrivateFile(listOf("users", "u1"), "auth.properties").toPath()
            Files.writeString(sourceFile, "refresh=redacted")

            val userDirectory = sourceRoot.resolve("users/u1")
            Files.setPosixFilePermissions(userDirectory, PosixFilePermissions.fromString("rwxr-xr-x"))
            assertFailsWith<IllegalStateException> {
                target.copyTrustedLegacyTreeFrom(sourceRoot.toFile())
            }
            assertTrue(target.isEmpty())

            Files.setPosixFilePermissions(userDirectory, PosixFilePermissions.fromString("rwx------"))
