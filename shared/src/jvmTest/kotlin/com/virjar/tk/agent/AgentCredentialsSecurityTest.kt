...
class AgentCredentialsSecurityTest {
    ...
    fun `credentials are private atomically replaced redacted and keep a stable device id`() {
        ...
        val firstIdentity = AgentCredentials.ensureIdentity(dataDir)
        val pending = AgentCredentials.beginRegistration(dataDir, "robot", "private-login-secret")
        ...
        val loaded = AgentCredentials.load(dataDir)!!
        val restartedIdentity = AgentCredentials.ensureIdentity(dataDir)
        ...
        assertEquals(AgentCredentialState.ACTIVE, AgentCredentials.requireActiveForInstall(verifiedDirectory).state)
    ...
    fun `systemd install gate rejects identity-only and pending credentials`() {
        ...
        AgentCredentials.ensureIdentity(identityOnly)
        assertFailsWith<IllegalArgumentException> {
            AgentCredentials.requireActiveForInstall(AgentDataDirectoryPolicy.openRuntime(identityOnly))
        }

        val pending = File(temporaryRoot(), "pending")
        AgentCredentials.beginRegistration(pending, "pending-agent", "pending-secret")
        assertFailsWith<IllegalArgumentException> {
            AgentCredentials.requireActiveForInstall(AgentDataDirectoryPolicy.openRuntime(pending))
        }
    }
    ...
    fun `active refresh rotation is atomic password-free and restart material is redacted`() {
        ...
        AgentCredentials.recordAuthentication(
            dataDir, "robot", identity.deviceId, "uid-robot", "robot", "refresh-one",
        )
        AgentCredentials.recordAuthentication(
            dataDir, "robot", identity.deviceId, "uid-robot", "robot", "refresh-two",
        )

        val restarted = AgentCredentials.load(dataDir)!!
    ...
    fun `legacy active plaintext password fails fast without rewriting the file`() {
        ...
        assertFailsWith<IllegalArgumentException> { AgentCredentials.load(dataDir) }
    ...
    fun `data directory symlink is rejected`() {
        ...
        assertFailsWith<IllegalArgumentException> {
            AgentCredentials.ensureIdentity(linkedDirectory)
        }
